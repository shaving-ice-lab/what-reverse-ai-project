...
3. [x] Phase 2：独立数据库与 DB 节点（输出：provision + db nodes；验收：可读写）  
   - 已完成 workspace DB provision + db_* 节点真实执行，支持读写与迁移。  
...
```json
{
  ...
  "nodes": [
    {
      ...
      "data": {
        "label": "开始",
        "inputs": [
          { "id": "prompt", "name": "prompt", "label": "提示词", "type": "string", "required": true }
        ],
        "config": {
          "form_id": "main_form",
  ...
  "edges": [
    { "id": "e1", "source": "start", "sourceHandle": "prompt", "target": "llm", "targetHandle": "prompt" },
    { "id": "e2", "source": "llm", "sourceHandle": "parsed", "target": "output", "targetHandle": "input" }
  ]
}
```
...
1. [x] 关键事件邮件/短信通知（输出：规则；验收：可执行）。  
   - 已实现：关键运行时事件触发邮件/短信通知并写入系统通知（覆盖 execution.failed、db.*.failed、domain.verify.failed、cert.issue.failed、quota.exceeded、security.auth_failed、system.error；邮件/短信需配置 `AGENTFLOW_SMTP_*` / `AGENTFLOW_TWILIO_*`）。  
