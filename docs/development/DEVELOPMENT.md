...
> **更新日期**: 2026-01-27  
> **基于PRD版本**: v1.0  
> **技术栈**: Next.js 16 + shadcn/ui + Golang Echo + Tauri
>
> **Phase 1.2 进度**: 可视化编辑器 - 36/36 完成 (100%) ✅
> **Phase 1.3 进度**: 基础节点集 - 21/21 完成 (100%) ✅
> **Phase 1.4 进度**: 执行引擎 - 14/14 完成 (100%) ✅
> **Phase 1.5 进度**: 用户系统 (前端) - 5/5 完成 (100%) ✅
> **Phase 1.6 进度**: 工作流管理 (前端) - 6/6 完成 (100%) ✅
> **Phase 1.7 进度**: 设置中心 (前端) - 6/6 完成 (100%) ✅
> **Phase 1.8 进度**: 后端核心服务 - 4/4 完成 (100%) ✅
> **Phase 1.9 进度**: 前端执行展示 - 4/4 完成 (100%) ✅
> **Phase 1.10 进度**: UX/性能优化 - 10/10 完成 (100%) ✅
> **Phase 1.11 进度**: Bug修复/稳定性 - 3/3 完成 (100%) ✅
> **Phase 1.12 进度**: 测试 - 4/4 完成 (100%) ✅
> **Phase 2.1 进度**: 节点 SDK - 9/9 完成 (100%) ✅
> **Phase 2.2 进度**: 插件系统 - 10/10 完成 (100%) ✅
...
> - ✅ 实现空状态组件 (EmptyState/SearchEmpty/CanvasEmpty)
...
> - ✅ 实现并行执行节点 (ParallelNode + ParallelExecutor)
> - ✅ 注册所有新执行器到引擎
>
> **更新 (v3.5)**:
...
> - ✅ 实现正则提取节点 (RegexNode + RegexExecutor + ReplaceExecutor)
> - ✅ 注册所有新执行器到引擎
>
> **更新 (v3.4)**:
...
> - ✅ 实现输出展示节点 (OutputNode 组件 + OutputExecutor)
> - ✅ 更新节点类型注册映射
>
> **更新 (v3.3)**:
...
> - ✅ 实现节点执行高亮效果 (useNodeExecutionStatus)
> - ✅ 更新组件和 Hook 导出
>
> **更新 (v3.2)**:
...
> - ✅ 实现 API 密钥加密存储 (AES-256-GCM/脱敏展示)
...
> - ✅ 完成配置面板实现 (ConfigPanel) - 支持 LLM/HTTP/Code 等节点配置
> - ✅ 完成顶部工具栏 (EditorToolbar)
> - ✅ 实现 10 种自定义节点组件: Start, End, LLM, HTTP, Condition, Loop, Code, Template, Variable, Base
> - ✅ 完成快捷键系统 (保存/撤销/重做/删除/全选)
> - ✅ 完成工作流列表页面 (workflows)
...
# 推荐 IDE
- VS Code / Cursor
  - 扩展: ESLint, Prettier, Go, Tailwind CSS IntelliSense
```
...
> 📋 **说明**：本任务清单采用详细的任务分解方式，每个任务包含验收标准、依赖关系、优先级和技术要点。
>
...
- [x] **FE-001: 创建 Next.js 16 项目** 🔴 P0 ✅
  ...
  - **技术要点**:
    ...
    - 启用 Turbopack 加速开发构建
    - 配置 `next.config.ts` 支持图片域名白名单

- [x] **FE-002: 配置 TypeScript 和路径别名** 🔴 P0 ✅
  - **验收标准**:
    - ✓ `@/*` 别名指向 `src/*`
    - ✓ `@components/*` 指向组件目录
    - ✓ 严格类型检查 `strict: true`
...
- [x] **FE-003: 安装配置 Tailwind CSS 4.x** 🔴 P0 ✅
  - **验收标准**:
    - ✓ Tailwind v4 正确安装
    - ✓ 主题色变量配置完成
    - ✓ 暗色模式支持
  - **技术要点**:
    - Tailwind v4 使用新的配置方式 (CSS-first)
    - 配置 CSS 变量实现主题切换
    - 集成 tailwind-merge 和 clsx

- [x] **FE-004: 安装配置 shadcn/ui 组件库** 🟠 P1 ✅
  - **验收标准**:
    - ✓ shadcn/ui CLI 初始化完成
    - ✓ 基础组件安装: Button, Input, Dialog, Toast, DropdownMenu
    - ✓ 组件样式与设计系统一致
  - **命令**: `pnpm dlx shadcn@latest init`
  - **需安装组件**:
    - 通用: Button, Badge, Avatar, Skeleton
    - 表单: Input, Textarea, Select, Checkbox, Switch, Form
    - 反馈: Toast, Dialog, AlertDialog, Tooltip
    - 导航: Tabs, DropdownMenu, Command, Menubar
    - 布局: Card, Separator, ScrollArea, Sheet

- [x] **FE-005: 配置 ESLint + Prettier** 🟠 P1 ✅
  - **验收标准**:
    - ✓ ESLint 规则配置完成
    - ✓ Prettier 格式化配置完成
    - ✓ VS Code 保存时自动格式化
    - ✓ Git pre-commit hook 配置 (lint-staged)
  - **依赖**: `eslint`, `prettier`, `eslint-config-next`, `@typescript-eslint/*`, `husky`, `lint-staged`

- [x] **FE-006: 安装配置 Zustand 状态管理** 🟠 P1 ✅
  - **验收标准**:
    - ✓ Zustand store 目录结构建立
    - ✓ 持久化中间件配置 (zustand/middleware)
    - ✓ DevTools 集成
  - **Store 结构**:
    ```
    src/stores/
    ...
    ├── useWorkflowStore.ts  # 工作流数据
    └── useUIStore.ts        # UI 状态 (主题、侧边栏等)
    ```

- [x] **FE-007: 安装配置 TanStack Query** 🟠 P1 ✅
  - **验收标准**:
    - ✓ QueryClientProvider 配置
    - ✓ 默认配置: staleTime, cacheTime, retry
    - ✓ DevTools 集成 (开发环境)
    - ✓ 错误边界处理
  - **技术要点**:
    - 配置全局错误处理
...
- [x] **BE-004: 配置 Redis 客户端** 🟠 P1 ✅
  - **验收标准**:
    - ✓ go-redis/v9 客户端配置
    - ✓ 连接池配置
    - ✓ 健康检查
    - ✓ 封装常用操作 (Get/Set/Del/Expire)
  - **用途**:
    - Session 存储
    - 缓存层
    - 分布式锁
...
- [x] **BE-010: 创建数据库迁移工具** 🟠 P1 ✅
  - **验收标准**:
    - ✓ golang-migrate 集成
    - ✓ 迁移命令封装 (up/down/version)
    - ✓ 迁移文件命名规范
  - **命令**:
    ```bash
    go run cmd/migrate/main.go up      # 执行迁移
    go run cmd/migrate/main.go down    # 回滚一个版本
...
- [x] **INFRA-001: 配置 Docker Compose** 🔴 P0 ✅
  - **验收标准**:
    - ✓ PostgreSQL 服务配置
    - ✓ Redis 服务配置
    - ✓ 数据持久化配置
...
- [x] **INFRA-002: 配置 GitHub Actions CI/CD** 🟠 P1 ✅
  - **验收标准**:
...
- [x] **CANVAS-001: 集成 React Flow 库** 🔴 P0 ✅
  - **验收标准**:
    - ✓ @xyflow/react 正确安装
    - ✓ 基础画布可渲染
    - ✓ 自定义节点类型注册
  ...
  - **技术要点**:
    - 使用 React Flow v12+
    - 配置 ReactFlowProvider
    - 实现自定义控制面板
  - **实现文件**: `src/components/editor/EditorCanvas.tsx`

- [x] **CANVAS-002: 实现无限画布** 🔴 P0 ✅
  - **验收标准**:
    - ✓ 缩放范围: 25% - 400%
    - ✓ 平移无边界限制
    - ✓ 双指/滚轮缩放支持
...
- [x] **CANVAS-006: 实现撤销/重做功能** 🟠 P1 ✅
  ...
  - **技术要点**:
    ...
    - 实现 Command 模式
    - 合并连续的同类操作
  - **实现文件**: `src/stores/useWorkflowStore.ts`
...
- [x] **NODE-002: 实现节点面板** 🔴 P0 ✅
  - **验收标准**:
    ...
    - ✓ 节点预览信息
    - ✓ 可折叠分类
  - **实现文件**: `src/components/editor/NodePanel.tsx`
  - **分类设计**:
    | 分类 | 图标 | 节点 |
    |---|---|---|
    | AI | 🤖 | LLM Chat, LLM Complete, Embedding |
    | 集成 | 🔌 | HTTP 请求, Webhook |
    | 逻辑 | ⚡ | 条件, 循环, 并行, 延迟, 错误处理 |
    | 数据 | 📊 | 变量, 转换, 合并, 筛选 |
    | 文本 | 📝 | 模板, 正则, 分割合并 |
...
- [x] **EDGE-001: 实现节点端口** 🔴 P0 ✅
  - **验收标准**:
    ...
    - ✓ 悬停提示端口信息
    - ✓ 端口连接状态显示
  - **实现文件**: `src/components/editor/nodes/BaseNode.tsx` 及各节点组件

- [x] **EDGE-003: 实现连线类型检查** 🟠 P1 ✅
  - **验收标准**:
    - ✓ 相同类型允许连接
    - ✓ any 类型可连接任何类型
    - ✓ 不兼容时显示错误提示
    - ✓ 防止自连接
    - ✓ 防止循环连接
  - **实现文件**: `src/components/editor/EditorCanvas.tsx` (增强 isValidConnection)
  - **类型兼容矩阵**:
...
- [x] **PANEL-005: 实现变量选择器** 🟠 P1 ✅
  ...
  - **变量来源**:
    ...
    - 循环变量 (在循环节点内)
    - 系统变量 (时间、执行ID等)
  - **实现文件**: `src/components/editor/VariableSelector.tsx`
...
- [x] **LAYOUT-001: 设计编辑器整体布局** 🔴 P0 ✅
  - **实现文件**: `src/components/editor/WorkflowEditor.tsx`
  - **布局结构**:
    ```
    ┌──────────────────────────────────────────────────────┐
    │                    顶部工具栏                         │
...
- [x] **LAYOUT-003: 实现自动布局算法** 🟡 P2 ✅
  ...
  - **技术要点**:
    ...
    - 配置节点间距参数
    - 动画过渡效果
  - **实现文件**: `src/hooks/useAutoLayout.ts`
...
- [x] **AI-001: LLM 调用节点 (Chat)** 🔴 P0 ✅
  - **验收标准**:
    - ✓ 模型选择 (OpenAI GPT-4/3.5, Claude 3, 本地模型)
    - ✓ System Prompt 配置 (多行文本)
    - ✓ User Prompt 配置 (支持变量插值 `{{var}}`)
    - ✓ 高级参数设置:
      - Temperature: 0-2 (默认 0.7)
      - Max Tokens: 1-32000 (默认 2048)
      - Top P: 0-1 (默认 1)
      - Frequency Penalty: 0-2 (默认 0)
      - Presence Penalty: 0-2 (默认 0)
    - ✓ 流式输出支持 (SSE)
    - ✓ 超时配置 (默认 60s)
    - ✓ 重试配置 (次数、间隔)
  - **输入端口**:
...
- [x] **INT-001: HTTP 请求节点** 🔴 P0 ✅
  - **验收标准**:
    - ✓ 方法选择: GET, POST, PUT, PATCH, DELETE
    - ✓ URL 配置 (支持变量)
    - ✓ Headers 配置 (键值对)
    - ✓ Query Params 配置
    - ✓ Body 配置:
  ...
  - **输入端口**: url (string), headers (object), body (any)
  - **输出端口**: data (any), status (number), headers (object)

- [x] **INT-002: Webhook 触发节点** 🟠 P1 ✅
  - **验收标准**:
    - ✓ 生成唯一 Webhook URL
    - ✓ 请求签名验证 (HMAC-SHA256)
    - ✓ 支持 GET/POST 方法
...
- [x] **LOGIC-001: 条件判断节点 (If/Else)** 🔴 P0 ✅
  - **验收标准**:
    - ✓ 多条件配置 (AND/OR)
    - ✓ 比较运算符: ==, !=, >, >=, <, <=, contains, startsWith, endsWith, matches (正则)
    - ✓ 逻辑运算符: AND, OR, NOT
    - ✓ 两个输出分支: true, false
    - ✓ 条件表达式模式 (高级)
  - **条件配置界面**:
...
- [x] **LOGIC-002: 循环节点 (Loop)** 🔴 P0 ✅
  - **验收标准**:
    - ✓ For Each 模式 (遍历数组)
    - ✓ While 模式 (条件循环)
    - ✓ 固定次数模式
    - ✓ 循环次数限制 (防止无限循环，默认 1000)
    - ✓ 循环内变量 (item, index)
    - ✓ break/continue 支持
  - **输入端口**: items (array), condition (boolean)
  - **输出端口**: item (any), index (number), results (array)
  - **循环体**: 作为子画布实现

- [x] **LOGIC-004: 等待节点 (Delay)** 🟡 P2 ✅
  - **验收标准**:
    - ✓ 固定延迟模式 (指定时间)
    - ✓ 随机延迟模式 (范围内随机)
    - ✓ 支持多种时间单位 (ms, s, m, h)
  ...
  - **配置选项**:
    - `duration`: 延迟时间
    - `unit`: 时间单位 (ms/s/m/h)
    - `mode`: 模式 (fixed/random)
    - `minDuration`: 随机模式最小值
    - `maxDuration`: 随机模式最大值
  - **实现文件**:
    - 前端: `apps/web/src/components/editor/nodes/DelayNode.tsx`
    - 后端: `apps/server/internal/pkg/executor/delay_executor.go`
...
- [x] **WS-001: 实现 WebSocket 服务** 🔴 P0 ✅
  ...
  - **消息类型**:
    ...
    - `execution.node.started` - 节点开始执行
    - `execution.node.completed` - 节点执行完成
    - `execution.node.failed` - 节点执行失败
    - `execution.log` - 执行日志
...
- [x] **AUTH-001: 实现邮箱注册** 🔴 P0 ✅
  ...
  - **API**: `POST /api/v1/auth/register`
    ...
    // Response
    { "message": "注册成功，请查收验证邮件", "user": { "id": "xxx", "email": "xxx" } }
    ```
...
│  │                       开发者 API 层                              │   │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────────┐ │   │
│  │  │ defineNode  │  │ Input/Output│  │ NodeContext (运行时)    │ │   │
│  │  │ (节点定义)  │  │ (类型系统)  │  │ - llm, http, cache...  │ │   │
│  │  └─────────────┘  └─────────────┘  └─────────────────────────┘ │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                              ↓                                          │
│  ┌─────────────────────────────────────────────────────────────────┐   │
  ...
  inputs: {
    ...
    style: Input.select({
      ...
      options: [
        ...
        { value: 'casual', label: '💬 口语化', description: '适合日常对话' },
        { value: 'technical', label: '🔧 技术文档', description: '保留专业术语' },
        { value: 'creative', label: '✨ 创意', description: '更自然流畅的表达' },
      ],
      default: 'formal',
    }),
    ...
    preserveFormatting: Input.boolean({
      label: '保留格式',
      default: true,
      description: '保留原文的换行和段落格式',
    }),

    glossary: Input.json({
      label: '术语表',
      required: false,
      description: '自定义术语映射，格式: {"原文": "译文"}',
      placeholder: '{"API": "应用程序接口"}',
    }),
  },
...
│   ├── inputs.ts             # Input 类型构建器
│   ├── outputs.ts            # Output 类型构建器
│   ├── context.ts            # NodeContext 接口
│   ├── validation.ts         # 验证相关
│   ├── errors.ts             # 错误类型
│   │
│   ├── runtime/              # 运行时实现
...
- [x] 实现节点加载器
- [x] 实现节点测试框架
- [x] 编写 CLI 工具 (init/build/test/publish)
- [x] 编写示例节点
- [x] 编写完整文档
- [ ] 发布 npm 包
...
│  │  {{ variable }}              变量插值                           │   │
│  │  {% if condition %}          条件语句                           │   │
│  │  {% for item in list %}      循环语句                           │   │
│  │  {{ value | filter }}        过滤器                             │   │
│  │  {# comment #}               注释                               │   │
│  │  {% include "partial" %}     模板包含                           │   │
│  │                                                                 │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                              ↓ 解析                                     │
│  ┌─────────────────────────────────────────────────────────────────┐   │
...
{{ items | reverse }}           {# 反转 #}
{{ items | sort }}              {# 排序 #}
{{ items | sort("name") }}      {# 按属性排序 #}
{{ items | unique }}            {# 去重 #}
{{ items | join(", ") }}        {# 连接 #}
{{ items | slice(0, 5) }}       {# 切片 #}
{{ items | map("name") }}       {# 提取属性 #}
{{ items | filter("active") }}  {# 过滤 #}
{{ items | groupBy("category") }} {# 分组 #}

{# JSON 过滤器 #}
{{ data | json }}               {# 转 JSON 字符串 #}
{{ data | json(2) }}            {# 格式化 JSON #}
{{ jsonStr | fromJson }}        {# 解析 JSON #}

{# 日期过滤器 #}
{{ date | formatDate("YYYY-MM-DD") }}
{{ date | relativeTime }}       {# 相对时间: "3天前" #}

{# AI 专用过滤器 #}
{{ text | tokenCount }}         {# Token 计数 #}
{{ text | summarize(100) }}     {# 摘要 (需 LLM) #}
{{ list | bulletPoints }}       {# 转为 bullet 列表 #}
{{ text | escape }}             {# 转义特殊字符 #}
{{ text | stripHtml }}          {# 去除 HTML 标签 #}
{{ text | markdown }}           {# Markdown 转文本 #}
...
## 翻译要求
1. 目标语言: {{ targetLang | languageName }}
2. 翻译风格: {{ style | styleName }}
3. {% if preserveFormatting %}保留原文格式{% else %}可调整格式{% endif %}
{% endblock %}

{# 用户消息 #}
{% block user %}
请翻译以下{{ sourceLang | languageName | default("") }}文本:
  ...
  async execute(code: string, inputs: Record<string, unknown> = {}): Promise<unknown> {
    ...
    try {
      ...
      if (error.message.includes('Script execution timed out')) {
        throw new SandboxError('TIMEOUT', '代码执行超时');
      }
      if (error.message.includes('Memory limit')) {
        throw new SandboxError('MEMORY_LIMIT', '内存使用超出限制');
      }
...
// PythonSandboxConfig Python 沙箱配置
type PythonSandboxConfig struct {
    ...
    AllowedPkgs   []string      `json:"allowed_pkgs"`   // 允许的包
}
...
- [ ] 实现 JavaScript 沙箱 (isolated-vm)
- [ ] 实现 Python 沙箱 (Docker 容器)
- [ ] 实现静态代码分析
- [ ] 实现资源限制监控
- [ ] 实现网络访问控制
- [ ] 实现文件系统隔离
...
- [x] 设计 Manifest 规范 ✅ `packages/sdk/src/plugin/manifest.ts`
- [x] 实现插件 API ✅ `packages/sdk/src/plugin/types.ts`
- [x] 实现插件加载器 ✅ `packages/sdk/src/plugin/loader.ts`
- [x] 实现插件沙箱 ✅ `packages/sdk/src/plugin/sandbox.ts`
- [x] 实现权限系统 ✅ `packages/sdk/src/plugin/permissions.ts`
- [x] 实现插件签名验证 ✅ `packages/sdk/src/plugin/signature.ts`
- [x] 实现插件市场 API ✅ `packages/sdk/src/plugin/marketplace.ts`
- [ ] 实现插件审核流程 (后端待实现)
- [x] 实现插件版本管理 ✅ `packages/sdk/src/plugin/version.ts`
- [x] 编写示例插件 ✅ `packages/sdk/src/plugin/examples/`
- [x] 编写开发者文档 ✅ `packages/sdk/src/plugin/PLUGIN_DEV_GUIDE.md`
...
Response:
  featured: Agent[]           # 精选推荐
  trending: Agent[]           # 热门趋势
  new_arrivals: Agent[]       # 最新上架
  categories: Category[]      # 热门分类
  collections: Collection[]   # 精选集合
...
Agent 组合器允许用户将多个 Agent 串联成更强大的"超级 Agent"，实现复杂的多步骤自动化任务。
...
Agent 进化系统通过用户反馈、A/B 测试和自动优化，持续改进 Agent 的性能和质量。
    ...
    for (const node of llmNodes) {
      ...
      const analysis = await this.llmService.chat({
        ...
        messages: [
          {
            role: 'system',
            content: `你是一位 Prompt 工程专家。请分析以下 Prompt 并提供优化建议。
关注以下方面：
1. 清晰度和具体性
2. 输出格式控制
3. 潜在的歧义或错误
...
(1, '新晋创作者', 0, 0.70, '{"features": ["basic_analytics"]}'),
(2, '活跃创作者', 1000, 0.72, '{"features": ["basic_analytics", "priority_review"]}'),
(3, '优质创作者', 5000, 0.75, '{"features": ["advanced_analytics", "priority_review", "featured_chance"]}'),
(4, '明星创作者', 20000, 0.78, '{"features": ["all_analytics", "instant_review", "featured_boost", "custom_page"]}'),
(5, '顶级创作者', 100000, 0.80, '{"features": ["all_features", "dedicated_support", "revenue_advance"]}');
```
...
│  │   ┌─────────────────────────────────────────────────────────┐  │   │
│  │   │                 Runtime Abstraction                      │  │   │
│  │   │   • StorageProvider   (本地存储 / 云端存储)              │  │   │
│  │   │   • LLMProvider       (本地模型 / 云端API)               │  │   │
│  │   │   • SyncProvider      (同步引擎)                         │  │   │
│  │   │   • ExecutionProvider (本地执行 / 云端执行)              │  │   │
│  │   └─────────────────────────────────────────────────────────┘  │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  ┌───────────────────────────┐  ┌───────────────────────────────────┐ │
...
│  │  │ SQLite              │  │  │  │ PostgreSQL                  │ │ │
│  │  │ 本地文件存储         │  │  │  │ Redis                       │ │ │
│  │  │ Ollama/LM Studio    │  │  │  │ OpenAI/Claude API           │ │ │
│  │  │ 本地执行引擎         │  │  │  │ 云端执行引擎                 │ │ │
│  │  └─────────────────────┘  │  │  └─────────────────────────────┘ │ │
│  └───────────────────────────┘  └───────────────────────────────────┘ │
│                                                                         │
                ...
                <div key={model.name} className="flex items-center justify-between p-3 border rounded">
                  <div>
                    ...
                    <div className="text-sm text-gray-500">
                      {formatBytes(model.size)} • {model.details?.parameterSize}
                    </div>
                  </div>
    ...
    <div className={cn(
      ...
      {isOnline && pendingChanges > 0 && (
        <>
          ...
          <span>正在同步 {pendingChanges} 项变更...</span>
        </>
      )}
    </div>
  );
