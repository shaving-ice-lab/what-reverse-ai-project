...
export const textTransformNode = defineNode({
  ...
  inputs: {
    ...
    trim: input.boolean('去除空白').default(true).description('是否去除文本首尾的空白字符').build(),
  },

  outputs: {
    result: output.string('转换结果').description('转换后的文本').build(),

    length: output.number('文本长度').description('转换后文本的字符数').build(),
  },
