...
export const httpRequestNode = defineNode({
  ...
  inputs: {
    url: input.string("请求 URL")
      .required()
      .url("请输入有效的 URL")
      .placeholder("https://api.example.com/data")
      .build(),
    
    method: input.select("请求方法", [
      { label: "GET", value: "GET" },
    ...
    useAuth: input.boolean("使用认证")
      .default(false)
      .description("是否使用 API 密钥认证")
      .build(),
    
    authHeader: input.string("认证头名称")
      .default("Authorization")
      .showIf({ field: "useAuth", operator: "equals", value: true })
      .build(),
    
    authSecretKey: input.string("密钥名称")
      .placeholder("API_KEY")
      .showIf({ field: "useAuth", operator: "equals", value: true })
  ...
  outputs: {
    data: output.any("响应数据")
      .description("HTTP 响应的数据内容")
      .build(),
    
    status: output.number("状态码")
      .description("HTTP 响应状态码")
      .build(),
    
    headers: output.object("响应头")
