...
export const jsonTransformNode = defineNode({
  ...
  inputs: {
    data: input.object("输入数据")
      .required()
      .json()
      .description("需要处理的 JSON 数据")
      .build(),
    
    operation: input.select("操作类型", [
  ...
  outputs: {
    result: output.any("处理结果")
      .description("转换后的数据")
      .build(),
    
    success: output.boolean("是否成功")
      .build(),
  },
