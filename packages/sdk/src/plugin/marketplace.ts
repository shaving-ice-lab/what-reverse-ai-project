...
export class MarketplaceClient {
  ...
  async downloadPlugin(id: string, version?: string): Promise<ArrayBuffer> {
    ...
    const response = await fetch(`${this.config.baseUrl}/plugins/${id}${versionPath}/download`, {
...
export class PublisherClient {
  ...
  async unpublish(pluginId: string, version?: string): Promise<boolean> {
    const path = version ? `/plugins/${pluginId}/versions/${version}` : `/plugins/${pluginId}`
  ...
  async updateMetadata(
    pluginId: string,
    metadata: Partial<
      Pick<MarketplacePlugin, 'description' | 'tags' | 'icon' | 'banner' | 'homepage'>
    >
  ): Promise<boolean> {
    try {
  ...
  async getDownloadStats(pluginId: string): Promise<Record<string, number>> {
    try {
      const response = await fetch(`${this.config.baseUrl}/plugins/${pluginId}/stats/downloads`, {
        headers: {
          Authorization: `Bearer ${this.config.apiKey}`,
        },
      })
