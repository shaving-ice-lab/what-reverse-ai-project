...
export async function activate(context: PluginContext, api: PluginAPI): Promise<void> {
  ...
  // 注册测试请求命令
  const testRequestCmd = api.commands.registerCommand('http-tools.testRequest', async () => {
  ...
  // 注册查看日志命令
  const viewLogsCmd = api.commands.registerCommand('http-tools.viewLogs', async () => {
  ...
  // 注册清除日志命令
  const clearLogsCmd = api.commands.registerCommand('http-tools.clearLogs', async () => {
    requestLogs = []
