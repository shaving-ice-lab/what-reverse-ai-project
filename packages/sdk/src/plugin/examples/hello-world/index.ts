...
export async function activate(context: PluginContext, api: PluginAPI): Promise<void> {
  ...
  // 注册 Say Hello 命令
  const sayHelloDisposable = api.commands.registerCommand('hello-world.sayHello', async () => {
  ...
  // 注册 Show Info 命令
  const showInfoDisposable = api.commands.registerCommand('hello-world.showInfo', async () => {
    const info = [
      `Plugin ID: ${pluginId}`,
      `Version: ${version}`,
