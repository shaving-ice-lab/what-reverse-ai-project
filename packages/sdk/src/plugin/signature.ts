...
export class SignatureVerifier {
  ...
  private parseCertificate(certPem: string): CertificateInfo {
    ...
    return {
      ...
      validTo: new Date(this.extractField(certPem, "notAfter") || Date.now() + 365 * 24 * 60 * 60 * 1000),
      fingerprint,
      publicKey: certPem,
    };
  }
  ...
  private isTrustedSigner(cert: CertificateInfo): boolean {
    ...
    return this.config.trustedRoots.some((root) => {
      ...
      return cert.issuer.includes(rootInfo.subject) || 
             cert.fingerprint === rootInfo.fingerprint;
    });
  }
