...
const confirmVariantStyles = {
  default: "bg-brand-500 hover:bg-brand-600 text-background",
  warning: "bg-warning-400 hover:bg-warning text-background",
  danger: "bg-destructive-400 hover:bg-destructive text-background",
}

const confirmIconStyles = {
  default: { icon: <Info className="w-6 h-6" />, variant: "info" as const },
  warning: { icon: <AlertTriangle className="w-6 h-6" />, variant: "warning" as const },
  danger: { icon: <AlertCircle className="w-6 h-6" />, variant: "error" as const },
}
...
function ConfirmDialog({
  ...
  const iconConfig = confirmIconStyles[variant]
  
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="sm" animation="scale" showClose={false}>
        <DialogHeader icon={iconConfig.icon} iconVariant={iconConfig.variant}>
          <DialogTitle className="text-center">{title}</DialogTitle>
          {description && (
            <DialogDescription className="text-center">{description}</DialogDescription>
          )}
