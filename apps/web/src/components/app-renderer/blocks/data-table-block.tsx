...
import { Search, ChevronLeft, ChevronRight, Pencil, Trash2, Eye, Check, X, Download, Plus } from 'lucide-react'
...
function buildSearchFilters(searchTerm: string, config: DataTableConfig) {
  ...
  if (config.search_key) {
    return { filters: [{ column: config.search_key, operator: 'LIKE', value: `%${searchTerm}%` }], combinator: undefined }
  }
  ...
  const filters = searchCols.map(col => ({ column: col, operator: 'LIKE', value: `%${searchTerm}%` }))
...
export function DataTableBlock({ config }: DataTableBlockProps) {
  const { queryRows, insertRow, updateRow, deleteRows, notifyTableChange, onTableChange } = useDataProvider()
  ...
  }, [queryRows, config.table_name, config.columns, pageSize, page, debouncedSearch, sortCol, sortDir])
  ...
  const handleDelete = async (row: Record<string, unknown>) => {
    const pkColumn = config.columns.find(col => col.key === 'id')?.key || config.columns[0]?.key || 'id'
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        else if (showCreateRow) { setShowCreateRow(false); setCreateValues({}) }
      }
    }
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      for (const col of config.columns) {
        if (col.key === 'created_at' || col.key === 'updated_at' || col.key === 'deleted_at') continue
  ...
  const handleCreateRow = async () => {
    ...
    try {
      const data: Record<string, unknown> = {}
      for (const col of config.columns) {
        if (col.key === 'id' || col.key === 'created_at' || col.key === 'updated_at' || col.key === 'deleted_at') continue
  ...
  const exportCSV = async () => {
    try {
      const { filters: exportFilters, combinator: exportCombinator } = buildSearchFilters(debouncedSearch, config)
    ...
    <div className="border border-border rounded-lg overflow-hidden">
      ...
      <div className="px-3 py-2 border-b border-border flex items-center gap-2 bg-surface-200/20">
          {config.search_enabled && (
            <div className="relative flex-1 max-w-xs">
              ...
              <Input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') { if (searchTimerRef.current) clearTimeout(searchTimerRef.current); setDebouncedSearch(search); setPage(0) } }}
                placeholder={config.search_key
                  ? `Search ${config.columns.find(c => c.key === config.search_key)?.label || config.search_key}...`
                  : `Search ${config.columns.filter(c => !c.type || c.type === 'text' || c.type === 'badge').map(c => c.label).slice(0, 3).join(', ') || 'records'}...`
                }
          ...
          {actions.includes('create') && (
            <Button size="sm" variant="ghost" className="h-7 gap-1 text-[10px] text-brand-500" onClick={() => { setShowCreateRow(!showCreateRow); setCreateValues({}) }}>
              <Plus className="w-3 h-3" />
              Add
            </Button>
          )}
          <Button size="sm" variant="ghost" className="h-7 gap-1 text-[10px] text-foreground-muted" onClick={exportCSV} disabled={rows.length === 0}>
      ...
      {showCreateRow && (
        <div className="px-3 py-3 border-b border-border bg-brand-500/5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => { setShowCreateRow(false); setCreateValues({}) }}>
              <X className="w-3.5 h-3.5" />
            </Button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {config.columns.filter((c) => c.key !== 'id' && c.key !== 'created_at' && c.key !== 'updated_at' && c.key !== 'deleted_at').map((col) => (
              <div key={col.key}>
                <label className="text-[10px] text-foreground-muted mb-0.5 block">{col.label}</label>
                {col.type === 'boolean' ? (
                  <input
                    type="checkbox"
                    checked={Boolean(createValues[col.key])}
                    onChange={(e) => setCreateValues((prev) => ({ ...prev, [col.key]: e.target.checked }))}
                  ...
                  <Input
                    ...
                    onChange={(e) => setCreateValues((prev) => ({ ...prev, [col.key]: e.target.value }))}
          ...
          <div className="flex justify-end mt-2 gap-1">
            <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => { setShowCreateRow(false); setCreateValues({}) }}>Cancel</Button>
      ...
      {viewRow && (
        <div className="px-4 py-3 border-b border-border bg-surface-200/30">
          <div className="flex items-center justify-between mb-2">
            ...
            <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => setViewRow(null)}>
              <X className="w-3.5 h-3.5" />
            </Button>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-1.5 max-h-48 overflow-y-auto">
            {config.columns.map((col) => (
              <div key={col.key}>
                ...
                <div className="text-xs text-foreground-light break-all">{formatCellValue(viewRow[col.key], col.type)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface-200/30">
              {config.columns.map((col) => (
                <th
                  key={col.key}
                  className={cn('text-xs font-medium text-foreground-muted px-3 py-2 border-b border-border cursor-pointer select-none hover:text-foreground transition-colors', col.type === 'number' ? 'text-right' : 'text-left')}
          ...
          <tbody>
            {loading ? (
                ...
                <tr key={`skel-${ri}`} className="animate-pulse">
                  {config.columns.map((col) => (
                    <td key={col.key} className="px-3 py-2.5 border-b border-border/50">
                      <div className={cn('h-3 bg-foreground/10 rounded', col.type === 'number' ? 'w-12 ml-auto' : 'w-24')} />
                    </td>
                  ))}
                  {actions.length > 0 && (
              ...
              <tr>
                <td colSpan={config.columns.length + (actions.length > 0 ? 1 : 0)} className="px-3 py-10 text-center">
                  <div className="text-xs text-foreground-muted">No records in <span className="font-medium">{config.table_name}</span></div>
                  <button onClick={fetchData} className="mt-2 text-[11px] text-brand-500 hover:underline">Refresh</button>
                </td>
              </tr>
            ) : (
              rows.map((row, i) => {
                ...
                return (
                  <tr key={i} className={cn('transition-colors', isEditing ? 'bg-brand-500/5' : 'hover:bg-surface-200/20')}>
                    {config.columns.map((col) => (
                      <td key={col.key} className={cn('px-3 py-2 border-b border-border/50 text-foreground-light max-w-[200px]', col.type === 'number' ? 'text-right tabular-nums' : col.type !== 'badge' && col.type !== 'boolean' ? 'truncate' : '')} title={col.type !== 'badge' && col.type !== 'boolean' && !isEditing ? String(row[col.key] ?? '') : undefined}>
                        {isEditing ? (
                          ['id', 'created_at', 'updated_at', 'deleted_at'].includes(col.key) ? (
                            ...
                            <input
                              ...
                              onChange={(e) => setEditValues((prev) => ({ ...prev, [col.key]: e.target.checked }))}
                              className="rounded border-border"
                            />
                          ) : (
                          <Input
                            type={col.type === 'number' ? 'number' : col.type === 'date' ? 'date' : 'text'}
                            value={col.type === 'date' ? toDateInputValue(editValues[col.key]) : String(editValues[col.key] ?? '')}
                    ...
                    {actions.length > 0 && (
                      <td className="px-3 py-2 border-b border-border/50 text-right">
                        <div className="flex items-center justify-end gap-1">
                          {isEditing ? (
                            <>
                              <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-brand-500" onClick={handleEditSave} disabled={saving}>
                            ...
                            <>
                              {actions.includes('view') && (
                                <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => setViewRow(row)}>
                                  <Eye className="w-3 h-3" />
                                </Button>
                              )}
                              {actions.includes('edit') && (
                                <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => handleEditStart(i, row)}>
...
function formatCellValue(value: unknown, type?: string): React.ReactNode {
  ...
  switch (type) {
    ...
    case 'badge': {
      ...
      const badgeColor = /active|enabled|online|approved|published/.test(v) ? 'bg-emerald-500/10 text-emerald-600'
        : /inactive|disabled|offline|archived/.test(v) ? 'bg-gray-500/10 text-gray-500'
        : /pending|review|waiting|draft/.test(v) ? 'bg-amber-500/10 text-amber-600'
        : /failed|error|rejected|blocked/.test(v) ? 'bg-red-500/10 text-red-600'
        : /completed|done|success|delivered/.test(v) ? 'bg-blue-500/10 text-blue-600'
        : 'bg-brand-500/10 text-brand-500'
      return <span className={cn('text-xs px-1.5 py-0.5 rounded', badgeColor)}>{String(value)}</span>
    }
    case 'number':
