 ...
 */
const cardVariants = cva('rounded-lg text-foreground transition-colors duration-150 ease-in-out', {
  variants: {
    variant: {
      // Default card - Supabase surface-100
      default: ['bg-surface-100 border border-border'].join(''),
      // Elevated card with shadow
      elevated: ['bg-surface-100 border border-border', 'shadow-md shadow-black/25'].join(''),
      // Outline card with transparent background
      outline: ['bg-transparent border border-border'].join(''),
      // Ghost card with no border
      ghost: ['bg-transparent border-0'].join(''),
      // Muted card - Supabase surface-75
      muted: ['bg-surface-75 border border-border-muted'].join(''),
      // Glass card with glass effect
      glass: ['bg-surface-100/70 backdrop-blur-xl border border-border/50'].join(''),
      // Glass card - more transparent
      'glass-subtle': ['bg-surface-100/40 backdrop-blur-lg border border-border/30'].join(''),
      ...
      panel: ['bg-surface-75 border border-border'].join(''),
      // Statistics card - Supabase dashboard style
      stats: ['bg-surface-100 border border-border'].join(''),
      // Brand highlight card with green background
      brand: ['bg-brand-200 border border-brand-400/30'].join(''),
      // Warning card
      warning: ['bg-warning-200 border border-warning/30'].join(''),
      // Destructive/error card
      destructive: ['bg-destructive-200 border border-destructive/30'].join(''),
    },
    hover: {
      none: '',
      ...
      interactive:
        'hover:border-border-strong hover:shadow-lg hover:shadow-black/20 hover:-translate-y-0.5',
    },
    padding: {
      none: '',
      xs: 'p-3',
...
const CardDescription = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('text-[12px] text-foreground-light', className)} {...props} />
  )
)
...
const GlowCard = React.forwardRef<HTMLDivElement, GlowCardProps>(
  ({ className, glowColor = '#3ECF8E', alwaysGlow = false, children, ...props }, ref) => {
    return (
...
function FlipCard({
  ...
  return (
    <div
      className={cn('relative perspective-1000', flipOnHover && 'group', className)}
      onClick={handleClick}
