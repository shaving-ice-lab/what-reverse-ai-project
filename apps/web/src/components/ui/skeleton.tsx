...
function Skeleton({ className, variant = 'pulse', rounded = 'md', ...props }: SkeletonProps) {
  const roundedStyles = {
    none: 'rounded-none',
    sm: 'rounded-sm',
    md: 'rounded-md',
  ...
  const variantStyles = {
    ...
    shimmer:
      'bg-surface-200 overflow-hidden relative before:absolute before:inset-0 before:-translate-x-full before:animate-shimmer before:bg-gradient-to-r before:from-transparent before:via-white/5 before:to-transparent',
    wave: 'bg-surface-200 animate-skeleton-wave',
  }

  return (
    <div className={cn(roundedStyles[rounded], variantStyles[variant], className)} {...props} />
  )
}
...
function SkeletonAvatar({ size = 'md', className, variant = 'pulse' }: SkeletonAvatarProps) {
  const sizes = {
    xs: 'w-6 h-6',
    sm: 'w-8 h-8',
    md: 'w-10 h-10',
  ...
  return <Skeleton variant={variant} rounded="full" className={cn(sizes[size], className)} />
}
...
function SkeletonCard({
  ...
  return (
    <div
      className={cn('bg-surface-100 border border-border rounded-xl overflow-hidden', className)}
    >
      {hasImage && <Skeleton variant={variant} rounded="none" className="w-full aspect-video" />}
  ...
  return (
    <div className={cn('w-full border border-border rounded-lg overflow-hidden', className)}>
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div
          key={rowIndex}
          className={cn('px-4 py-3 flex gap-4', rowIndex < rows - 1 && 'border-b border-border')}
        >
...
function SkeletonStats({ count = 4, className, variant = 'pulse' }: SkeletonStatsProps) {
  return (
...
function SkeletonForm({
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      {hasButton && <Skeleton variant={variant} className="h-10 w-full rounded-lg mt-6" />}
    </div>
  )
}
