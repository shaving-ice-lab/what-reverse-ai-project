    ...
    <AccordionPrimitive.Header className="flex">
      ...
      >
        <div className="flex items-center gap-3">
          {leftIcon && <span className="shrink-0 text-foreground-muted">{leftIcon}</span>}
          <div className="text-left">
            <div>{children}</div>
            {subtitle && (
              <div className="text-xs text-foreground-muted font-normal mt-0.5">{subtitle}</div>
            )}
          </div>
        </div>
    ...
    <Accordion type="single" collapsible className={className}>
      {items.map((item, index) => (
        <AccordionItem key={index} value={`faq-${index}`} variant="card">
          ...
          <AccordionContent variant="card">{item.answer}</AccordionContent>
        </AccordionItem>
      ))}
    </Accordion>
  )
