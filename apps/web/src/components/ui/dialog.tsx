...
>(
  (
    { className, children, position, animation, size, showClose = true, overlayBlur, ...props },
    ref
  ) => {
    return (
      <DialogPortal>
        ...
        <DialogPrimitive.Content
          ref={ref}
          className={cn(contentVariants({ position, animation, size }), 'p-6', className)}
          {...props}
        >
          {children}
          {showClose && (
...
const DialogHeader = ({
  ...
  <div
    className={cn('flex flex-col mb-4', icon && 'items-center text-center', className)}
    {...props}
  >
    {icon && (
      <div
...
const drawerSideStyles = {
  ...
  right:
    'right-0 top-0 h-full rounded-l-2xl rounded-r-none data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right',
  ...
  bottom:
    'bottom-0 left-0 w-full rounded-t-2xl rounded-b-none data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom',
}
