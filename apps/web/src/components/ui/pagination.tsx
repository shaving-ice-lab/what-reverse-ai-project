...
function Pagination({
  ...
  const getButtonStyles = (isActive: boolean, isDisabled: boolean) => {
    ...
    if (variant === 'ghost') {
      return cn(base, isActive ? 'bg-primary text-primary-foreground' : 'hover:bg-surface-200')
    }

    if (variant === 'simple') {
      return cn(
        base,
        isActive ? 'text-primary font-semibold' : 'text-foreground-light hover:text-foreground'
      )
    }
...
function PaginationInfo({ page, pageSize, total, className }: PaginationInfoProps) {
  const start = (page - 1) * pageSize + 1
  const end = Math.min(page * pageSize, total)

  return (
    <p className={cn('text-sm text-foreground-light', className)}>
      Display <span className="font-medium text-foreground">{start}</span> -{' '}
  ...
  return (
    <div className={cn('flex flex-col sm:flex-row items-center justify-between gap-4', className)}>
      ...
      <div className="flex items-center gap-4">
        {showInfo && <PaginationInfo page={page} pageSize={pageSize} total={total} />}
...
export { Pagination, PaginationInfo, PageSizeSelect, FullPagination, SimplePagination }
export type {
  PaginationProps,
  PaginationInfoProps,
  PageSizeSelectProps,
