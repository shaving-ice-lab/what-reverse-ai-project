...
>(({ className, inset, icon, shortcut, destructive, description, children, ...props }, ref) => (
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <div>{children}</div>
      {description && (
        <p className="text-[11px] text-foreground-muted mt-0.5 font-normal">
          {description}
        </p>
      )}
    </div>
...
const DropdownMenuShortcut = ({
  ...
  return (
    <span
      className={cn(
        "ml-auto text-xs text-foreground-muted tracking-widest",
        className
      )}
      {...props}
    >
  ...
  return (
    <div className={cn("flex items-center gap-3 px-2 py-2", className)}>
      ...
      <div className="min-w-0">
        ...
        {email && (
          <p className="text-[11px] text-foreground-light truncate">{email}</p>
        )}
      </div>
    </div>
  )
...
/**
 * DropdownMenuIconItem - 图标菜单项（简化版）
 */
