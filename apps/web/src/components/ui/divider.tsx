...
function DividerWithText({
  ...
  return (
    <div
      role="separator"
      className={cn('flex items-center gap-3', spacingClass[spacing || 'default'], className)}
      {...props}
    >
      ...
      {children && (
        <span className="text-xs text-foreground-light whitespace-nowrap px-2">{children}</span>
      )}
...
function SectionDivider({ title, description, action, className }: SectionDividerProps) {
  return (
    <div className={cn('py-4', className)}>
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="text-sm font-medium text-foreground">{title}</h3>
          {description && <p className="text-xs text-foreground-light mt-0.5">{description}</p>}
        </div>
...
function ListDivider({ inset = false, insetSize = 48, className }: ListDividerProps) {
  return (
    <Divider
      className={className}
      spacing="none"
