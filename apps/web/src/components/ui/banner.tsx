  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        {title && <p className={cn('font-medium', config.textColor)}>{title}</p>}
        {description && <p className={cn('text-sm opacity-80', config.textColor)}>{description}</p>}
        {children}
      </div>
      ...
      {action &&
        (action.href ? (
          ...
          <Button variant="ghost" size="sm" className={config.textColor} onClick={action.onClick}>
            {action.label}
            <ArrowRight className="w-4 h-4 ml-1" />
          </Button>
        ))}
          ...
          {features && features.length > 0 && (
            <ul className="flex flex-wrap gap-3 mb-4">
              {features.map((feature, index) => (
                <li key={index} className="flex items-center gap-1.5 text-sm text-white/90">
...
export function CookieBanner({ onAccept, onDecline, onCustomize }: CookieBannerProps) {
  const [isDismissed, setIsDismissed] = useState(false)
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-4">
        <div className="flex-1 text-center md:text-left">
          <p className="text-sm text-foreground">
            We use cookies to improve your browsing experience, provide personalized content, and
            analyze website traffic. By continuing to use this website, you agree to our use of
            cookies.
          </p>
        </div>
...
export function MaintenanceBanner({ scheduledTime, duration, message }: MaintenanceBannerProps) {
  return (
    <Banner
      variant="warning"
      icon={AlertTriangle}
