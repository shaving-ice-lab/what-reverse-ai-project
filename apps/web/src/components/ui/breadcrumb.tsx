...
function Breadcrumb({
  ...
  const visibleItems = React.useMemo(() => {
    ...
    return [
      ...
      <BreadcrumbEllipsis
        key="ellipsis"
        items={items.slice(itemsBeforeCollapse, -itemsAfterCollapse)}
      />,
      ...afterItems,
    ]
...
function BreadcrumbItem({ className, isCurrent = false, children, ...props }: BreadcrumbItemProps) {
  return (
    <li
...
function BreadcrumbPage({ className, icon, children, ...props }: BreadcrumbPageProps) {
  return (
    <span
      className={cn('flex items-center gap-1.5 text-foreground font-medium', className)}
      aria-current="page"
      {...props}
    >
      {icon}
...
function BreadcrumbSeparator({
  ...
  return (
    <li className={cn('text-muted-foreground/50', className)} aria-hidden {...props}>
...
function BreadcrumbWithDropdown({ label, icon, items }: BreadcrumbWithDropdownProps) {
  const [isOpen, setIsOpen] = React.useState(false)
