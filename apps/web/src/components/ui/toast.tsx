...
import { CheckCircle2, XCircle, AlertCircle, Info, X, Loader2, type LucideIcon } from 'lucide-react'
...
const typeConfig: Record<
  ToastType,
  { icon: LucideIcon; className: string; iconColor: string; progressColor: string }
> = {
  success: {
    icon: CheckCircle2,
...
export function ToastProvider({
  ...
  const updateToast = useCallback((id: string, config: Partial<Omit<ToastConfig, 'id'>>) => {
    setToasts((prev) => prev.map((t) => (t.id === id ? { ...t, ...config } : t)))
  }, [])
...
/**
 * ToastInitializer - Initialall toast
 * at ToastProvider InternalUsage
 */
