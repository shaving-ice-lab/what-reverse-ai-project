...
export function StarRating({
  ...
  return (
    <div className={cn('flex items-center', gapClasses[size], className)}>
      ...
      {showValue && <span className="text-sm text-muted-foreground ml-2">{value.toFixed(1)}</span>}
    </div>
  )
}
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      >
        ...
        {showCounts && (
          <span className="text-sm">{dislikes + (userVote === 'dislike' ? 1 : 0)}</span>
        )}
      </button>
    </div>
  )
...
export function HeartButton({
  ...
  return (
    ...
    >
      ...
      />
      {showCount && (
        <span className="text-sm text-muted-foreground">{count + (liked ? 1 : 0)}</span>
      )}
    </button>
  )
}
...
export function EmojiRating({ value, onChange, showLabels = false, className }: EmojiRatingProps) {
  const options = [
    {
      id: 'positive',
      icon: Smile,
      ...
      {options.map((option) => {
        ...
        return (
          ...
          >
            ...
            {showLabels && <span className="text-xs font-medium">{option.label}</span>}
          </button>
        )
      })}
    </div>
...
export function RatingDisplay({ rating, reviews, breakdown, className }: RatingDisplayProps) {
