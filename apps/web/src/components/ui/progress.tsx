    ...
    <div className={cn('w-full', className)}>
      ...
      {(label || showValue) && (
        <div className="flex items-center justify-between mb-1.5">
          ...
          {showValue && (
            <span className="text-sm tabular-nums text-foreground-muted">{displayValue}</span>
          )}
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {children ||
          (showValue && (
            <span className="text-sm font-medium tabular-nums text-foreground">{displayValue}</span>
          ))}
      </div>
    </div>
  )
...
function SegmentedProgress({
  ...
  return (
    <div className={cn('w-full', className)}>
      {/* Progress Bar */}
      <div
        className={cn('flex w-full rounded-full overflow-hidden bg-surface-200', sizeMap[size])}
        style={{ gap }}
      >
      ...
      {steps.map((step, index) => {
        ...
        return (
          <div
            ...
            className={cn(
              'flex',
              direction === 'vertical' ? 'flex-row items-start' : 'flex-col items-center flex-1'
            )}
          >
            <div className={cn('flex items-center', direction === 'vertical' && 'flex-col')}>
              {/* Circle */}
              <div
                className={cn(
            ...
            {/* Labels */}
            <div className={cn(direction === 'vertical' ? 'ml-3' : 'mt-2 text-center', 'min-w-0')}>
              <div
                className={cn(
                  'font-medium text-sm',
...
export { Progress, CircularProgress, SegmentedProgress, StepProgress, ProgressRingGroup }
export type {
  ProgressProps,
  CircularProgressProps,
  SegmentedProgressProps,
