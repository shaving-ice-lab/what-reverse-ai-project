...
const badgeVariants = cva(
  cn('inline-flex items-center gap-1 font-medium', 'border transition-all duration-150'),
  {
    variants: {
      variant: {
        // Supabase default/primary: Brand Green
        default: ['bg-surface-200', 'border-border', 'text-foreground-light'].join(''),
        primary: ['bg-brand-200', 'border-brand-400/30', 'text-brand-500'].join(''),
        // Supabase secondary: Gray
        secondary: ['bg-surface-200', 'border-border', 'text-foreground-light'].join(''),
        // Supabase success: Green
        success: ['bg-brand-200', 'border-brand-400/30', 'text-brand-500'].join(''),
        // Supabase warning: Orange
        warning: ['bg-warning-200', 'border-warning/30', 'text-warning'].join(''),
        // Supabase error/destructive: Red
        error: ['bg-destructive-200', 'border-destructive/30', 'text-destructive'].join(''),
        destructive: ['bg-destructive-200', 'border-destructive/30', 'text-destructive'].join(''),
        // info: Blue
        info: ['bg-blue-500/15', 'border-blue-500/30', 'text-blue-400'].join(''),
        // violet: Purple
        violet: ['bg-violet-500/15', 'border-violet-500/30', 'text-violet-400'].join(''),
        // pink: Pink
        pink: ['bg-pink-500/15', 'border-pink-500/30', 'text-pink-400'].join(''),
        // Supabase outline style
        outline: ['bg-transparent', 'border-border', 'text-foreground-light'].join(''),
        // solid Variant - Supabase Style
        'solid-primary': ['bg-brand-500', 'border-brand-500', 'text-[#171717]'].join(''),
        'solid-secondary': ['bg-surface-300', 'border-surface-300', 'text-foreground'].join(''),
        'solid-destructive': ['bg-destructive', 'border-destructive', 'text-white'].join(''),
        // gradient Variant
        gradient: [
...
function Badge({
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      {icon && !dot && <span className="shrink-0 -ml-0.5">{icon}</span>}

      {/* Content or count */}
      {displayCount !== null ? <span className="tabular-nums">{displayCount}</span> : children}
...
function StatusBadge({ status, label, showDot = true, className }: StatusBadgeProps) {
  ...
  return (
    ...
    >
      {showDot && (
        <span
          className={cn('w-2 h-2 rounded-full', status === 'online' && 'animate-pulse')}
