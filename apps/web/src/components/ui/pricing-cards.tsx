    ...
    >
      ...
      <ul className="flex-1 space-y-3 mb-6">
        {features.map((feature, index) => (
          <li
            key={index}
            className={cn('flex items-start gap-2 text-sm', !feature.included && 'opacity-50')}
          >
            {feature.included ? (
      ...
      <Button
        ...
        className={cn('w-full', popular && 'bg-primary hover:bg-primary/90')}
        disabled={current}
        onClick={onSelect}
      >
...
export function PlanComparison({ plans, features, className }: PlanComparisonProps) {
  return (
  ...
  return (
    <div
      className={cn(
        ...
        highlighted ? 'bg-primary/5 border-primary' : 'bg-card border-border',
        className
      )}
    >
