...
import { GitBranch, Tag, ChevronDown, ChevronUp, Sparkles, Bug, Zap, FileText } from 'lucide-react'
...
const getChangeTypeIcon = (changelog: string) => {
  ...
  if (
    lowerChangelog.includes('new feature') ||
    lowerChangelog.includes('add') ||
    lowerChangelog.includes('feature')
  ) {
    return { icon: Sparkles, color: 'text-primary', bg: 'bg-primary/10' }
  }
  if (
    lowerChangelog.includes('Fix') ||
    lowerChangelog.includes('bug') ||
    lowerChangelog.includes('fix')
  ) {
    return { icon: Bug, color: 'text-orange-500', bg: 'bg-orange-500/10' }
  }
  if (
    lowerChangelog.includes('optimize') ||
    lowerChangelog.includes('improve') ||
    lowerChangelog.includes('enhance')
  ) {
...
export function VersionHistory({
  ...
  if (!versions || versions.length === 0) {
    return (
      <div
        className={cn(
          'rounded-xl bg-surface-200/50 border border-border p-6 text-center',
          className
        )}
      >
