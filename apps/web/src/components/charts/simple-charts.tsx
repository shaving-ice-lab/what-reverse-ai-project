    ...
    <div className={cn('w-full', className)}>
      ...
      {showLabels && (
        <div className="flex justify-between gap-2 mt-2">
          {data.map((item, index) => (
            <span key={index} className="flex-1 text-xs text-muted-foreground text-center truncate">
              {item.label}
            </span>
          ))}
        </div>
  ...
  return (
    <div className={cn('w-full', className)}>
      ...
      >
        ...
        {/* Fill region */}
        <path d={areaD} className={cn(fillColor, 'transition-all duration-500')} />
  ...
  return (
    <div className={cn('flex items-center gap-6', className)}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        {donut && (
          <circle cx={radius} cy={radius} r={innerRadius - 5} className="fill-background" />
        )}
      </svg>
