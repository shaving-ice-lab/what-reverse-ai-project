      ...
      <PopoverContent className="w-72 p-0" align="end">
          ...
          {filteredVariables.nodeVars.length === 0 &&
            filteredVariables.systemVars.length === 0 && (
              <div className="p-6 text-center text-foreground-muted">
                ...
                <p className="text-sm">没有找到变量</p>
                <p className="text-xs mt-0.5">连接上游节点以获取变量</p>
              </div>
            )}
        </ScrollArea>
        ...
        <div className="p-2 border-t border-border bg-surface-200">
          <p className="text-xs text-foreground-muted text-center">
            使用 <code className="font-mono bg-surface-200 px-1 py-0.5 rounded border border-border">{"{{变量}}"}</code> 引用
          </p>
        </div>
      </PopoverContent>
    </Popover>
