...
import { Braces, Search, Hash, Type, ToggleLeft, Box, List, Asterisk, Clock } from 'lucide-react'
...
export function VariableSelector({ onSelect, currentNodeId, className }: VariableSelectorProps) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-72 p-0" align="end">
        ...
        <ScrollArea className="h-[280px]">
                ...
                {filteredVariables.systemVars.map((variable) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-foreground-muted font-mono">{variable.path}</p>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {filteredVariables.nodeVars.length === 0 && filteredVariables.systemVars.length === 0 && (
        ...
        <div className="p-2 border-t border-border bg-surface-200">
          <p className="text-xs text-foreground-muted text-center">
            Use{' '}
            <code className="font-mono bg-surface-200 px-1 py-0.5 rounded border border-border">
              {'{{Variable}}'}
            </code>{' '}
