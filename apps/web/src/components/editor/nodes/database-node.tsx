    ...
    >
      ...
      <div className="flex items-center gap-2.5 border-b border-border/70 px-3 py-2.5 bg-brand-200/30">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-sm truncate">{data.label || "数据库操作"}</h3>
        ...
        {operationKey === "migrate" ? (
          ...
          <>
            <div className="flex items-center gap-2">
              <span className="px-1.5 py-0.5 rounded bg-surface-200 text-foreground-muted">
                表
              </span>
            ...
            {where && (
              <div className="text-foreground-muted truncate">
                条件: <span className="font-mono">{where}</span>
              </div>
            )}
            {typeof config.limit === "number" && operationKey === "select" && (
              <div className="text-foreground-muted">
                限制: <span className="font-mono">{config.limit}</span>
              </div>
            )}
