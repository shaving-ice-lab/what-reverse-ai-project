    ...
    >
      ...
      <div className="flex items-center gap-2.5 border-b border-border/70 px-3 py-2.5 bg-brand-200/30">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-sm truncate">{data.label || "数据筛选"}</h3>
      ...
      <div className="px-3 py-2.5 space-y-2 text-xs">
        {config.field && (
          <div className="flex items-center gap-2">
            <span className="text-foreground-muted">字段:</span>
        ...
        {filterType === "range" && (
          <div className="text-foreground-muted px-1.5 py-1 rounded bg-surface-200">
            范围: <span className="font-mono">{config.minValue ?? "-∞"}</span> ~ <span className="font-mono">{config.maxValue ?? "+∞"}</span>
          </div>
        )}
        {config.pattern && (
