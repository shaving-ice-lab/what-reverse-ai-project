...
export function CollaboratorPanel({ workflowId, maxDisplay = 4 }: CollaboratorPanelProps) {
  const { lastMessage, isConnected } = useWebSocketContext()
  const [collaborators, setCollaborators] = useState<Collaborator[]>([])

  // Process collaborator message
  const handleCollaboratorMessage = useCallback(
    (message: unknown) => {
      ...
      switch (msg.type) {
        ...
        case 'collaborator.left':
          setCollaborators((prev) =>
            prev
              .map((c) => (c.id === msg.payload.user_id ? { ...c, is_online: false } : c))
        ...
        case 'collaborator.cursor':
          if (msg.payload.cursor) {
            setCollaborators((prev) =>
              prev.map((c) =>
                c.id === msg.payload.user_id ? { ...c, cursor: msg.payload.cursor } : c
              )
            )
          }
          break
        ...
        case 'collaborator.selection':
          if (msg.payload.selection) {
            setCollaborators((prev) =>
              prev.map((c) =>
                c.id === msg.payload.user_id ? { ...c, selection: msg.payload.selection } : c
              )
            )
          }
          break
