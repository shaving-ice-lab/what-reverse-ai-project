...
function JsonViewer({ data, title, defaultOpen = false, maxHeight = '200px' }: JsonViewerProps) {
      ...
      <CollapsibleContent>
        ...
        >
          <pre className="p-3 text-xs text-foreground-muted font-mono whitespace-pre-wrap break-all">
            {isEmpty ? <span className="text-foreground-muted italic">No data</span> : jsonString}
          </pre>
        </ScrollArea>
      </CollapsibleContent>
    </Collapsible>
    ...
    <div className={cn('flex flex-col h-full', className)}>
      ...
      <div className="shrink-0 p-4 border-b border-border">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-medium text-foreground">{snapshot.nodeName}</h3>
              <div className="flex items-center gap-2 mt-1">
                ...
                >
                  <StatusIcon
                    className={cn('w-3 h-3 mr-1', snapshot.status === 'running' && 'animate-spin')}
                  />
                  {statusConfig.label}
                </Badge>
              </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {snapshot.metadata?.tokensUsed && (
            <div className="p-2.5 rounded-lg bg-surface-200">
              ...
              {snapshot.metadata.promptTokens && snapshot.metadata.completionTokens && (
          ...
          {snapshot.metadata?.httpStatusCode && (
            <div className="p-2.5 rounded-lg bg-surface-200">
              ...
              <p
                className={cn(
                  'text-sm font-medium',
                  snapshot.metadata.httpStatusCode >= 400 ? 'text-destructive' : 'text-brand-500'
                )}
              >
                {snapshot.metadata.httpStatusCode}
              </p>
        ...
        <div className="mt-3 flex items-center gap-4 text-[10px] text-foreground-muted">
          ...
          {snapshot.completedAt && <span>Done: {formatDateTime(snapshot.completedAt)}</span>}
        </div>
      </div>
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-4">
          ...
          {snapshot.error && (
            <div className="p-3 rounded-lg bg-destructive-200 border border-destructive/30">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-destructive">Execution Error</span>
                {snapshot.error.code && (
                  <Badge
                    variant="secondary"
          ...
          {/* Input Data */}
          <JsonViewer data={snapshot.inputs} title="Input data" defaultOpen={true} />
