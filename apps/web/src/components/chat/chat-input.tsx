    ...
    <div className={cn('border-t border-border bg-background-studio/95 backdrop-blur', className)}>
      ...
      <div className="py-4">
        ...
        >
          ...
          {/* Send/Stop Button */}
          {isLoading ? (
            <Button size="icon" variant="outline" className="h-9 w-9 shrink-0" onClick={onCancel}>
              <X className="w-5 h-5" />
            </Button>
          ) : (
            <Button
...
export function TypingIndicator({ isTyping, userName = 'AI', className }: TypingIndicatorProps) {
  if (!isTyping) return null

  return (
    <div
      className={cn('flex items-center gap-2 px-4 py-2 text-sm text-foreground-muted', className)}
    >
