...
export interface ChatMessage {
  ...
  parentId?: string; // 回复的消息 ID
  parentMessage?: ChatMessage; // 引用的消息内容（用于显示）
}
          ...
          <div className="flex items-start gap-2 p-2 rounded-md bg-surface-100 border border-border border-l-brand-500/40">
            ...
            <div className="min-w-0">
              <span className="text-xs text-foreground-muted">
                回复 {message.parentMessage.role === "user" ? "你" : "AI"}：
              </span>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {message.attachments.map((attachment, index) => (
              ...
              >
                ...
                {attachment.type === "file" && <FileText className="w-4 h-4 text-foreground-muted" />}
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon-xs" className="text-foreground-muted hover:text-foreground">
                  <MoreHorizontal className="w-4 h-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="bg-surface-100 border-border">
                <DropdownMenuItem onClick={onReply} className="text-foreground-light hover:text-foreground hover:bg-surface-200">
                ...
                {isUser && (
                  <>
                    ...
                    <DropdownMenuItem onClick={onEdit} className="text-foreground-light hover:text-foreground hover:bg-surface-200">
                ...
                <DropdownMenuSeparator className="bg-border" />
                <DropdownMenuItem onClick={onDelete} className="text-destructive-400 hover:bg-destructive-200">
      ...
      {isLoading && !streamingMessageId && (
        <div className="flex gap-4 px-6 py-5 bg-surface-100/70">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-foreground-light">AI 正在思考...</span>
          </div>
        </div>
      )}
...
export function WelcomeMessage({
  title = "你好！我是 AI 助手",
  description = "我可以帮助你完成各种任务，比如回答问题、写作、编程等。试着问我些什么吧！",
  suggestions = [],
  onSuggestionClick,
  className,
}: WelcomeMessageProps) {
    ...
    <div className={cn("flex w-full flex-col items-center justify-center py-10", className)}>
      <div className="w-full max-w-3xl space-y-6">
        ...
        <div className="flex items-center justify-between rounded-lg border border-border bg-surface-75 px-4 py-3 text-xs text-foreground-muted">
          <span>提示：支持上传文件、语音输入与快捷回复</span>
          <span className="text-brand-500">Enter 发送，Shift + Enter 换行</span>
        </div>
      </div>
    </div>
  );
