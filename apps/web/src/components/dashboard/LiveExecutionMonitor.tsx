...
interface LiveExecutionMonitorProps {
  className?: string;
  workflowId?: string; // 可选，指定只监控特定工作流
}
        ...
        setWorkflows((prev) => {
          ...
          return [
            ...prev,
            {
              ...
              name: `工作流 ${execPayload.workflowId?.slice(-6) || "Unknown"}`,
              icon: "🔄",
              ...
              ? {
                  ...
                  logs: [
                    ...wf.logs.slice(-50),
                    {
                      ...
                      timestamp: new Date(timestamp),
                      level: "info" as const,
                      message: `节点 ${nodePayload.nodeType || nodePayload.nodeId} 开始执行`,
              ...
              ? {
                  ...
                  logs: [
                    ...wf.logs.slice(-50),
                    {
                      ...
                      message: `节点 ${nodePayload.nodeType || nodePayload.nodeId} 完成 (${nodePayload.durationMs}ms)`,
                      node: nodePayload.nodeType || nodePayload.nodeId,
                    },
                  ],
                }
              ...
              ? {
                  ...
                  logs: [
                    ...wf.logs.slice(-50),
                    {
                      ...
                      message: `节点 ${nodePayload.nodeType || nodePayload.nodeId} 失败: ${nodePayload.error}`,
                      node: nodePayload.nodeType || nodePayload.nodeId,
                    },
                  ],
                }
              ...
              ? {
                  ...
                  logs: [
                    ...wf.logs.slice(-50),
                    {
                      ...
                      timestamp: new Date(timestamp),
                      level: "success" as const,
                      message: `执行完成 (总耗时: ${execPayload.durationMs}ms)`,
                    },
                  ],
                }
              : wf
              ...
              ? {
                  ...
                  logs: [
                    ...wf.logs.slice(-50),
                    {
                      ...
                      timestamp: new Date(timestamp),
                      level: "error" as const,
                      message: `执行失败: ${execPayload.error}`,
                    },
                  ],
                }
              : wf
      ...
      <div className="flex items-center justify-between p-4 border-b border-border/50 bg-gradient-to-r from-amber-500/5 via-transparent to-transparent">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {runningCount > 0 ? `${runningCount} 个工作流正在执行` : "暂无执行中的任务"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className={cn(
            ...
            <span>{isConnected ? "已连接" : connectionState === "connecting" ? "连接中" : "未连接"}</span>
          </div>
          {!isConnected && (
            <Button
              variant="ghost"
      ...
      <div className="max-h-[400px] overflow-auto">
        {isLoading ? (
          <div className="p-10 flex flex-col items-center justify-center">
            ...
            <p className="text-sm font-medium text-muted-foreground">加载执行记录...</p>
          </div>
        ) : workflows.length === 0 ? (
          <div className="p-10 text-center">
            ...
            <p className="text-sm font-semibold text-foreground">暂无执行中的任务</p>
            <p className="text-xs text-muted-foreground mt-1.5">
              {isConnected ? (
                <span className="flex items-center justify-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                  WebSocket 已连接，等待执行事件...
                </span>
              ) : (
                "请检查 WebSocket 连接"
              )}
