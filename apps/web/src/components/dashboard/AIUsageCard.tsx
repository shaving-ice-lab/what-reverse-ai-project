 ...
 * AI 用量监控卡片
 * 
 * Token 消耗统计、按模型分布（GPT-4/Claude/本地LLM）、用量趋势和预警
 * 使用 Stats API 获取真实数据
 */
...
const defaultModelUsageData: ModelUsage[] = [
  ...
  { name: "本地 LLM", tokens: 0, cost: 0, color: "#F59E0B", icon: "🟡" },
];
...
export function AIUsageCard({ className }: AIUsageCardProps) {
  ...
  const [overviewData, setOverviewData] = useState<ReturnType<typeof formatOverviewStats> | null>(null);
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      if (overviewResponse.data) {
        ...
        if (totalTokens > 0) {
          setModelUsageData([
            { name: "GPT-4", tokens: Math.floor(totalTokens * 0.15), cost: totalTokens * 0.15 * 0.0001, color: "#10B981", icon: "🟢" },
            { name: "GPT-3.5", tokens: Math.floor(totalTokens * 0.45), cost: totalTokens * 0.45 * 0.00001, color: "#3B82F6", icon: "🔵" },
            { name: "Claude 3", tokens: Math.floor(totalTokens * 0.1), cost: totalTokens * 0.1 * 0.0001, color: "#8B5CF6", icon: "🟣" },
            { name: "本地 LLM", tokens: Math.floor(totalTokens * 0.3), cost: 0, color: "#F59E0B", icon: "🟡" },
          ]);
        }
      }
    ...
    <Card className={cn("border border-border/60 bg-card/80 backdrop-blur-sm p-6 hover:border-violet-500/20 transition-colors duration-300", className)}>
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-foreground">AI 用量监控</h3>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="p-4 rounded-xl bg-gradient-to-br from-primary/5 to-transparent ring-1 ring-primary/10 hover:ring-primary/20 transition-all duration-300 group">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-muted-foreground">Token 消耗</span>
            <div className={cn(
            ...
            <div className="h-2 bg-muted/50 rounded-full overflow-hidden ring-1 ring-border/30">
              <div
                className={cn(
                  ...
                  stats.tokenUsagePercent > 80 ? "bg-gradient-to-r from-amber-500 to-amber-400" : "bg-gradient-to-r from-primary to-primary/90"
                )}
        ...
        <div className="p-4 rounded-xl bg-gradient-to-br from-violet-500/5 to-transparent ring-1 ring-violet-500/10 hover:ring-violet-500/20 transition-all duration-300 group">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-muted-foreground">预估费用</span>
          ...
          <div className="mt-3">
            ...
            <div className="h-2 bg-muted/50 rounded-full overflow-hidden ring-1 ring-border/30">
              <div
                className={cn(
                  "h-full rounded-full transition-all duration-500 relative",
                  stats.budgetUsagePercent > 80 ? "bg-gradient-to-r from-amber-500 to-amber-400" : "bg-gradient-to-r from-violet-500 to-violet-400"
                )}
            ...
            return (
              ...
              >
                ...
                <span className={cn(
                  ...
                  isToday ? "text-violet-500 font-medium" : "text-muted-foreground group-hover:text-foreground"
                )}>
                  {data.date.slice(1)}
                </span>
              </div>
                ...
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-popover/95 backdrop-blur-sm border border-border/50 rounded-xl px-3 py-2 text-xs opacity-0 group-hover:opacity-100 transition-all duration-200 whitespace-nowrap z-10 shadow-xl scale-90 group-hover:scale-100">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-muted-foreground">{(model.tokens / 1000).toFixed(0)}K tokens</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {modelUsageData.map((model, index) => (
              ...
              >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {stats.totalTokens > 0 ? ((model.tokens / stats.totalTokens) * 100).toFixed(1) : 0}%
                      </span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className={cn(
                    "text-xs",
                    model.cost > 0 ? "text-amber-500" : "text-primary"
                  )}>
                    {model.cost > 0 ? `$${model.cost.toFixed(2)}` : "免费"}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1">
            {modelUsageData.map((model, index) => (
              ...
              >
                ...
                />
                <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">{model.name}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {stats.isWarning && (
        <div className="mt-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-amber-500">用量预警</p>
              <p className="text-xs text-muted-foreground mt-1">
                本月 Token 使用量已达 {stats.tokenUsagePercent.toFixed(0)}%，建议合理规划使用或升级套餐。
              </p>
            </div>
          </div>
        </div>
