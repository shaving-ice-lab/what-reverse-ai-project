...
Each agent can run independently, saving you a lot of time!`
  ...
  if (
    ...
    lowerQuestion.includes('Knowledge Base')
  ) {
...
export function FloatingAssistant() {
  ...
  const renderContent = (content: string) => {
    return content
      ...
      .replace(
        /\[([^\]]+)\]\(([^)]+)\)/g,
        '<a href="$2" class="text-primary hover:underline cursor-pointer" data-href="$2">$1</a>'
      )
      .replace(/\n\n/g, '</p><p>')
      .replace(/\n/g, '<br />')
      .replace(/^- /gm, '• ')
      ...
      {isOpen && (
        <div
          className={cn(
            ...
            isMinimized ? 'bottom-6 right-6 w-72 h-14' : 'bottom-6 right-6 w-[380px] h-[520px]'
          )}
        >
          {/* Header */}
