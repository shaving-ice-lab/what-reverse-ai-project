...
export function ExecutionTrendChart({ className }: ExecutionTrendChartProps) {
  ...
  const timeRangeConfig = {
    "7d": { label: "7 天", days: 7 },
    "30d": { label: "30 天", days: 30 },
    "90d": { label: "90 天", days: 90 },
  };
    ...
    <Card className={cn("border border-border/60 bg-card/80 backdrop-blur-sm p-6 hover:border-primary/20 transition-colors duration-300", className)}>
      ...
      >
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-muted-foreground">加载趋势数据...</p>
          </div>
        ) : error ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">执行工作流后会显示趋势</p>
          </div>
        ) : (
          <>
            {/* SVG 图表 */}
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="text-sm">
                    ...
                    总执行: <span className="font-semibold text-foreground">{displayData[hoveredIndex].current}</span>
                  </p>
                  <p className="text-xs text-muted-foreground">
                    成功: {displayData[hoveredIndex].success} / 失败: {displayData[hoveredIndex].failed}
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-border/50">
        <div className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-muted/50 cursor-pointer transition-colors group">
          ...
          <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">执行次数</span>
        </div>
        <div className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-muted/50 cursor-pointer transition-colors group">
          <div className="w-3 h-3 rounded-md bg-primary/30 ring-1 ring-primary/20 group-hover:scale-125 transition-transform" />
          <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">成功</span>
        </div>
        <div className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-muted/50 cursor-pointer transition-colors group">
          <div className="w-3 h-3 rounded-md bg-red-500/30 ring-1 ring-red-500/20 group-hover:scale-125 transition-transform" />
          <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">失败</span>
        </div>
      </div>
    </Card>
  );
