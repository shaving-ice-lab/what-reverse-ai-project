...
export function LiveExecutionMonitor({ className, workflowId }: LiveExecutionMonitorProps) {
  ...
  const loadRunningExecutions = useCallback(async () => {
    try {
      ...
      const runningWorkflows: ExecutingWorkflow[] = (response.data || []).map(
            ...
            prev.map((wf) =>
              wf.executionId === nodePayload.executionId
                ? {
                    ...
                    status:
                      nodePayload.completedNodes === nodePayload.totalNodes
                        ? 'completing'
                        : wf.status,
                    logs: [
                      ...wf.logs.slice(-50),
  ...
  const { connectionState, isConnected, connect, disconnect, subscribe, unsubscribe } =
    useWebSocket({
  ...
  return (
    <Card
      className={cn(
        'border border-border/60 bg-card/80 backdrop-blur-sm overflow-hidden hover:border-amber-500/20 transition-colors duration-300',
        className
      )}
    >
      {/* Header - Enhanced */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {runningCount > 0
                ? `${runningCount} ${runningCount === 1 ? 'workflow' : 'workflows'} running`
                : 'No running tasks'}
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          workflows.map((workflow, index) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  setExpandedId(expandedId === workflow.executionId ? null : workflow.executionId)
                }
              ...
              >
                ...
                <div className="flex-1 min-w-0 text-left">
                  <div className="flex items-center gap-2 mb-2">
                    <h4 className="font-semibold text-foreground truncate group-hover:text-amber-500 transition-colors">
                      {workflow.name}
                    </h4>
                    <span
                      className={cn(
                    ...
                    {workflow.logs.map((log, logIndex) => (
                      <div
                        ...
                        style={{
                          ...
                          animation:
                            logIndex >= workflow.logs.length - 3
                              ? 'fadeInUp 200ms ease-out both'
                              : 'none',
                        }}
                      >
                        ...
                        {log.node && (
                          <span className="text-blue-400 shrink-0 px-1.5 py-0.5 rounded bg-blue-500/10 text-[10px]">
                            {log.node}
                          </span>
                        )}
                        <span className={cn(getLogLevelStyle(log.level), 'flex-1')}>
                          {log.message}
                        </span>
                      </div>
                    ))}
