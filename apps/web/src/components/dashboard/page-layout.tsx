...
export function PageContainer({ children, className, fullWidth = false }: PageContainerProps) {
  return (
    ...
    >
      <div className={cn(!fullWidth && 'max-w-[1280px] mx-auto w-full')}>{children}</div>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {description && <p className="text-[12px] text-foreground-light mt-1">{description}</p>}
      </div>
...
export function CategoryHeader({ children, className }: CategoryHeaderProps) {
  return <div className={cn('page-caption mb-2', className)}>{children}</div>
}
...
export function StatsCard({ icon, title, value, subtitle, trend, className }: StatsCardProps) {
  return (
    <div className={cn('bg-surface-100 border border-border rounded-lg p-4', className)}>
      ...
      {subtitle && <p className="text-sm text-foreground-light mt-1">{subtitle}</p>}
    </div>
  )
}
...
export function SidebarNavItem({
  ...
  return (
    ...
    >
      ...
      {external && <span className="text-foreground-muted">↗</span>}
    </Link>
  )
}
...
export function SidebarNavGroup({ title, children }: SidebarNavGroupProps) {
  return (
    <div className="mb-4">
      {title && <CategoryHeader className="px-3 mb-1.5">{title}</CategoryHeader>}
