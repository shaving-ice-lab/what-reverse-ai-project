  ...
  return (
    <div className={cn('rounded-2xl bg-card border border-border overflow-hidden', className)}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        <Button
          size="sm"
          className="bg-primary/10 hover:bg-primary/20 text-primary border-0 gap-1.5"
        >
      ...
      <div className="divide-y divide-border max-h-[400px] overflow-y-auto">
        {filteredTasks.length === 0 ? (
          ...
          filteredTasks.map((task) => {
            ...
            return (
              <div key={task.id} className="p-4 hover:bg-muted/50 transition-all group">
                <div className="flex items-start gap-3">
                  {/* Status Button */}

                  <button onClick={() => toggleTaskStatus(task.id)} className="mt-0.5 shrink-0">
                    {task.status === 'completed' ? (
      ...
      <div className="flex items-center justify-between p-3 border-t border-border bg-muted/30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            AI can help you plan and track tasks
          </span>
        </div>
