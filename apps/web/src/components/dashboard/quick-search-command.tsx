 ...
 * 命令面板式快速搜索组件
 * 
 * 全局搜索工作流（Cmd+K）、最近访问+收藏列表、快速操作（运行/编辑/复制）
 */
...
const recentItems: SearchResult[] = [
  { id: "1", name: "客服自动回复", icon: "🤖", type: "workflow", description: "使用 AI 自动回复客户问题", lastAccess: new Date() },
  { id: "2", name: "数据同步任务", icon: "🔄", type: "workflow", description: "每日自动同步数据", lastAccess: new Date(Date.now() - 3600000) },
  { id: "3", name: "邮件分类助手", icon: "📧", type: "workflow", description: "自动分类和标记邮件", lastAccess: new Date(Date.now() - 7200000) },
];

const favoriteItems: SearchResult[] = [
  { id: "1", name: "客服自动回复", icon: "🤖", type: "workflow", isFavorite: true },
  { id: "4", name: "内容生成器", icon: "✨", type: "workflow", isFavorite: true },
];
...
const allWorkflows: SearchResult[] = [
  ...recentItems,
  { id: "4", name: "内容生成器", icon: "✨", type: "workflow", description: "批量生成营销文案" },
  { id: "5", name: "数据分析报告", icon: "📊", type: "workflow", description: "自动生成数据分析报告" },
  { id: "6", name: "社交媒体监控", icon: "📱", type: "workflow", description: "实时监控社交媒体提及" },
  { id: "7", name: "订单处理流程", icon: "🛒", type: "workflow", description: "自动处理电商订单" },
];
        ...
        {selectedAction && (
          <div className="border-b border-border/50 p-4 bg-gradient-to-r from-muted/30 via-transparent to-muted/30">
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                { action: "run" as const, icon: Play, label: "运行", color: "text-primary", bg: "bg-primary/10 hover:bg-primary/20", ring: "ring-primary/20" },
                { action: "edit" as const, icon: Edit, label: "编辑", color: "text-blue-500", bg: "bg-blue-500/10 hover:bg-blue-500/20", ring: "ring-blue-500/20" },
                { action: "copy" as const, icon: Copy, label: "复制", color: "text-purple-500", bg: "bg-purple-500/10 hover:bg-purple-500/20", ring: "ring-purple-500/20" },
                { action: "delete" as const, icon: Trash2, label: "删除", color: "text-red-500", bg: "bg-red-500/10 hover:bg-red-500/20", ring: "ring-red-500/20" },
                  ...
                  {searchResults.length > 0 ? (
                    <>
                      <div className="px-2 py-1.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                        ...
                        搜索结果
                        <span className="ml-auto text-primary">{searchResults.length} 个</span>
                      </div>
                    ...
                    <div className="py-12 text-center">
                      ...
                      <p className="text-xs text-muted-foreground/60 mt-1">尝试使用不同的关键词搜索</p>
                    </div>
                  )}
                </div>
              )}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border/50 bg-gradient-to-r from-muted/30 via-transparent to-muted/30 text-xs text-muted-foreground">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5">
              ...
              <span className="text-muted-foreground/70">导航</span>
            </span>
            <span className="flex items-center gap-1.5">
              <kbd className="px-1.5 py-0.5 rounded-md bg-muted/50 font-mono text-[10px] ring-1 ring-border/50">↵</kbd>
              <span className="text-muted-foreground/70">选择</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-primary/5 ring-1 ring-primary/10">
            ...
            <span className="text-muted-foreground/60">打开搜索</span>
          </div>
        </div>
