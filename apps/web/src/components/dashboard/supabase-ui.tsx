...
/**
 * Supabase Dashboard StyleuseComponent
 * Contains: PageHeader, SecondarySidebar, EmptyState, TabNav, Badge etc
 */
  ...
  return (
    <aside className="w-[220px] shrink-0 border-r border-border bg-background-studio overflow-y-auto">
      ...
      <div className="sticky top-0 bg-background-studio z-10">
        ...
        {headerContent && <div className="px-3 py-3 border-b border-border">{headerContent}</div>}
      </div>
      ...
      <div className="p-3 space-y-4">
        {groups.map((group, groupIndex) => (
          <div key={groupIndex}>
            {group.title && <div className="px-2 py-2 page-caption">{group.title}</div>}
...
export function EmptyState({ icon: Icon, title, description, action, className }: EmptyStateProps) {
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center py-16 text-center', className)}>
      ...
      <p className="text-[13px] text-foreground-muted max-w-sm mb-4">{description}</p>
      {action &&
        (action.href ? (
          <Link
            href={action.href}
