...
export function ExecutionTrendChart({ className }: ExecutionTrendChartProps) {
  ...
  return (
    <Card
      className={cn(
        'border border-border/60 bg-card/80 backdrop-blur-sm p-6 hover:border-primary/20 transition-colors duration-300',
        className
      )}
    >
      {/* Header */}
      <div className="flex items-start justify-between mb-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Success Rate {stats.successRate}%</p>
        </div>
      ...
      {/* Chart Region */}
      <div className="relative h-48" onMouseLeave={() => setHoveredIndex(null)}>
        {isLoading ? (
          ...
          <>
            ...
            <svg viewBox="0 0 100 100" className="w-full h-full" preserveAspectRatio="none">
              ...
              {displayData.map((point, index) => {
                const x =
                  5 +
                  (displayData.length > 1 ? index * ((100 - 10) / (displayData.length - 1)) : 45)
            ...
            {hoveredIndex !== null && displayData[hoveredIndex] && (
              <div
                ...
                style={{
                  ...
                  top: '10%',
                  transform:
                    hoveredIndex > displayData.length / 2 ? 'translateX(-100%)' : 'translateX(0)',
                }}
              >
                ...
                <div className="space-y-1">
                  <p className="text-sm">
                    ...
                    <span className="font-semibold text-foreground">
                      {displayData[hoveredIndex].current}
                    </span>
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Success: {displayData[hoveredIndex].success} / Failed:{' '}
                    {displayData[hoveredIndex].failed}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="absolute bottom-0 left-0 right-0 flex justify-between px-1">
              {displayData
                .filter(
                  (_, i) =>
                    i % Math.ceil(displayData.length / 5) === 0 || i === displayData.length - 1
                )
                .map((point, index) => (
