...
export function NotificationCenter({ className, compact = false }: NotificationCenterProps) {
  ...
  const convertWSNotification = (n: WSNotification): Notification => {
    ...
    return {
      ...
      actionLabel: n.executionId ? "查看执行详情" : undefined,
    };
  };

  // 合并 WebSocket 通知和本地通知
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-foreground">通知中心</h3>
              {wsContext && (
                <span className={cn(
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-sm text-muted-foreground",
                "bg-muted/50 hover:bg-muted hover:text-foreground transition-all duration-200",
                "ring-1 ring-transparent hover:ring-border/50"
              )}
            >
              <Filter className="w-3.5 h-3.5" />
              <span className="text-xs font-medium">
                {filter === "all" ? "全部" : getTypeLabel(filter as NotificationType)}
              </span>
              ...
              <div className="absolute right-0 top-full mt-2 bg-popover/95 backdrop-blur-sm border border-border/50 rounded-xl shadow-2xl overflow-hidden z-10 min-w-[140px] ring-1 ring-white/5">
                {(["all", "alert", "info", "update", "success"] as const).map((type, index) => (
                  ...
                  >
                    ...
                    <span className="font-medium">{type === "all" ? "全部" : getTypeLabel(type as NotificationType)}</span>
                  </button>
                ))}
              </div>
            )}
      ...
      <div className={cn("overflow-auto", compact ? "max-h-64" : "max-h-96")}>
        {filteredNotifications.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <p className="text-xs text-muted-foreground/60 mt-1">系统消息会显示在这里</p>
          </div>
        ) : (
