...
import {
  useWebSocketContext,
  type Notification as WSNotification,
...
export function NotificationCenter({ className, compact = false }: NotificationCenterProps) {
  ...
  return (
    <Card
      className={cn(
        'border border-border/60 bg-card/80 backdrop-blur-sm overflow-hidden hover:border-rose-500/20 transition-colors duration-300',
        className
      )}
    >
      {/* Header */}
                ...
                {(['all', 'alert', 'info', 'update', 'success'] as const).map((type, index) => (
                  ...
                  >
                    ...
                    <span className="font-medium">
                      {type === 'all' ? 'All' : getTypeLabel(type as NotificationType)}
                    </span>
                  </button>
                ))}
              </div>
