...
console.log(result.output);`,
    output: `{
  "success": true,
  "output": "您好！我来帮您查询订单状态。请提供您的订单号，我将立即为您查询。",
  "tokens": 45,
  "duration": "1.2s"
}`,
  },
...
export function LiveCodePreview({
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        "grid",
        showOutput && showResult ? "lg:grid-cols-2" : "grid-cols-1"
      )}>
        {/* Code Panel */}
...
export function SimpleCodeBlock({
  ...
  const highlightCode = (line: string) => {
    return line
      .replace(/\b(import|from|const|let|var|function|return|await|async|class|new)\b/g, '<span class="text-violet-400">$1</span>')
      .replace(/(["'`])(?:(?!\1)[^\\]|\\.)*?\1/g, '<span class="text-emerald-400">$&</span>')
      .replace(/\/\/.*/g, '<span class="text-muted-foreground/60 italic">$&</span>');
  };
