...
const resultTypeConfig: Record<SearchResultType, {
  ...
  workflow: { icon: Zap, label: "工作流", color: "text-amber-500", bg: "bg-amber-500/10" },
  agent: { icon: Bot, label: "Agent", color: "text-purple-500", bg: "bg-purple-500/10" },
  conversation: { icon: MessageSquare, label: "对话", color: "text-blue-500", bg: "bg-blue-500/10" },
  document: { icon: FileText, label: "文档", color: "text-emerald-500", bg: "bg-emerald-500/10" },
  team: { icon: Users, label: "团队", color: "text-pink-500", bg: "bg-pink-500/10" },
  setting: { icon: Settings, label: "设置", color: "text-muted-foreground", bg: "bg-muted/50" },
  action: { icon: Command, label: "操作", color: "text-cyan-500", bg: "bg-cyan-500/10" },
};
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center py-12", className)}>
      ...
      <h3 className="text-lg font-medium text-foreground mb-2">
        {query ? `没有找到 "${query}" 的结果` : "开始搜索"}
      </h3>
      <p className="text-sm text-muted-foreground text-center max-w-sm">
        {query
          ? "尝试使用不同的关键词或检查拼写"
          : "搜索工作流、Agent、对话、文档等内容"}
      </p>
    </div>
  );
}
