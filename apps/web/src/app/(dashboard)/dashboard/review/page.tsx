...
export default function ReviewQueuePage() {
  ...
  const handleBatchApprove = async () => {
    ...
    const confirmed = await confirm({
      title: "批量通过审核",
      description: `确定要通过选中的 ${selectedIds.size} 个审核项目吗？`,
      confirmText: "确认通过",
      cancelText: "取消",
    });
  ...
  const handleBatchReject = async () => {
    ...
    const confirmed = await confirm({
      title: "批量拒绝审核",
      description: `确定要拒绝选中的 ${selectedIds.size} 个审核项目吗？`,
      confirmText: "确认拒绝",
      cancelText: "取消",
      variant: "destructive",
    });
  ...
  const statusTabs = [
    ...
    { label: "审核中", value: "in_review", count: stats?.inReview },
    { label: "已通过", value: "approved", count: stats?.approved },
    { label: "已拒绝", value: "rejected", count: stats?.rejected },
    { label: "需修改", value: "revision", count: stats?.revision },
  ];
  ...
  return (
    <PageContainer className="space-y-6">
      ...
      <PageHeader
        ...
        title="Review Queue"
        description="管理和审核待发布的 Agent、工作流和模板"
        badge={(
      ...
      <div className="space-y-3">
        {error && (
          <EmptyState
            ...
            title="加载失败"
            description={error}
            action={{ label: "重试", onClick: loadReviews }}
          />
        )}
        ...
        {!isLoading && !error && reviews.length === 0 && (
          <EmptyState
            ...
            title={hasFilters ? "没有找到匹配的审核项目" : "暂无审核项目"}
            description={
              hasFilters
                ? "尝试调整筛选条件"
                : "当用户提交 Agent 或工作流发布请求时，审核项目会显示在这里"
            }
          />
        )}
            ...
            {isBatchMode && (
              <Card variant="muted" padding="sm" className="flex items-center gap-3">
                ...
                <span className="text-[13px] text-foreground-light">
                  {selectedIds.size === reviews.length ? "取消全选" : "全选当前页"}
                </span>
              </Card>
            )}
