...
const agents: Agent[] = [
  ...
  },
  {
    ...
    name: 'CodeReviewBot',
    description:
      'Automatically review pull requests, check code standards, provide improvement suggestions',
    status: 'paused',
    model: 'Claude 3',
    type: 'code-review',
...
export default function MyAgentsPage() {
  ...
  const selectedVisibleCount = sortedAgents.filter((agent) => selectedSet.has(agent.id)).length
  const allVisibleSelected = sortedAgents.length > 0 && selectedVisibleCount === sortedAgents.length
  const someVisibleSelected = selectedVisibleCount > 0 && selectedVisibleCount < sortedAgents.length
  ...
  return (
    <PageWithSidebar sidebarWidth="narrow" sidebarTitle="Agents" sidebar={sidebar}>
      <PageContainer>
