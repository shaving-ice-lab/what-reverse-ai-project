...
import { PageWithSidebar, SidebarNavGroup } from '@/components/dashboard/page-layout'
        ...
        {knowledgeBases.map((kb) => (
          ...
          >
            ...
            {kb.status === 'indexing' ? (
              ...
              <span className="text-[11px] text-foreground-muted shrink-0">{kb.fileCount}</span>
            )}
          </button>
        ))}
    ...
    >
      <div className="space-y-6 max-w-[960px]">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[18px] font-semibold text-foreground">File Management</h1>
          ...
          <div className="p-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div className="p-3 rounded-md border border-border bg-surface-75/60">
                <p className="text-[11px] text-foreground-muted mb-1">Total Files</p>
                <p className="text-lg font-semibold text-foreground">{stats.total}</p>
                <p className="text-[11px] text-foreground-muted">{stats.starred} favorited</p>
              </div>
              <div className="p-3 rounded-md border border-border bg-surface-75/60">
                <p className="text-[11px] text-foreground-muted mb-1">Indexed</p>
                <p className="text-lg font-semibold text-foreground">{stats.indexed}</p>
              ...
              <div className="p-3 rounded-md border border-border bg-surface-75/60">
                ...
                <p className="text-lg font-semibold text-foreground">{stats.knowledgeBases}</p>
                <p className="text-[11px] text-foreground-muted">
                  {(stats.totalVectors / 1000).toFixed(1)}k vectors
                </p>
              </div>
              <div className="p-3 rounded-md border border-border bg-surface-75/60">
                <p className="text-[11px] text-foreground-muted mb-1">Storage Usage</p>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-md bg-surface-200/80 border border-border flex items-center justify-center shrink-0">
                      {(() => {
                        ...
                        return <FileIcon className="w-3.5 h-3.5 text-foreground-light" />
                      })()}
                    </div>
        ...
        <div className="page-panel">
          <div className="page-panel-header flex items-center justify-between">
            <div>
              <h2 className="page-panel-title">{activeFolder?.name || 'File'}</h2>
                  ...
                  {sortedFiles.map((file) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => isSelectionMode && toggleSelect(file.id)}
                      >
                        {/* File Info */}
                        <div className="flex items-center gap-3 min-w-0">
                          {isSelectionMode && (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <DropdownMenu>
                            ...
                            >
                              ...
                              <DropdownMenuItem icon={<Edit3 />} className="cursor-pointer">
                                Rename
                              </DropdownMenuItem>
                              <DropdownMenuItem icon={<Star />} className="cursor-pointer">
                                {file.starred ? 'Unfavorite' : 'Favorite'}
                              </DropdownMenuItem>
                              <DropdownMenuItem icon={<Brain />} className="cursor-pointer">
                                Add to Knowledge Base
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem icon={<Share2 />} className="cursor-pointer">
                                Share
                              </DropdownMenuItem>
                              <DropdownMenuItem icon={<LinkIcon />} className="cursor-pointer">
                                Copy Link
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-[11px] text-foreground-muted lg:hidden">
                          ...
                          {file.knowledgeBase && <span>{file.knowledgeBase}</span>}
                        </div>
                      </div>
                    )
                  })}
                  ...
                  return (
                    ...
                    >
                      ...
                      <h3 className="text-[13px] font-medium text-foreground mb-1 flex items-center gap-2">
                        {file.starred && <Star className="w-3.5 h-3.5 text-warning fill-current" />}
