...
const resolveAppWorkspaceId = (app?: App | null) => app?.id || null
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const requiredPermissions = useMemo(() => resolveRequiredPermissions(pathname), [pathname])
  ...
  if (status !== 'ready' || !workspaceId || !appId) {
    ...
    return (
      <ExceptionState
        variant={
          status === 'not_found' ? 'not_found' : status === 'forbidden' ? 'permission' : 'error'
        }
        title={message}
