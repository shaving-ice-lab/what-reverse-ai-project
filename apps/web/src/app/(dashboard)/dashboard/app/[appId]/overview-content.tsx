...
export function AppOverviewPageContent({ workspaceId, appId }: AppOverviewPageProps) {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true)
      setLoadError(null)
      const [workspaceData, appData, metricsData, accessPolicyData, domainsData, executionsData] =
        await Promise.all([
  ...
  const accessMode = accessPolicy?.access_mode || app?.access_policy?.access_mode || 'private'
  ...
  const domainHint = primaryDomain
    ? `Primary Domain: ${primaryDomain}`
    : 'Bind a custom domain to enhance brand recognition'
  ...
  const errorRate =
    metrics && metrics.total_executions ? metrics.failure_count / metrics.total_executions : 0
      ...
      >
        <PageContainer>
          <PageHeader
            ...
            description={
              app?.description || 'View app basic information, status summary, and run metrics.'
            }
            backHref="/dashboard/apps"
            backLabel="Back to App List"
            badge={
              app ? (
                <Badge variant="secondary" className={cn('gap-1', status.bgColor, status.color)}>
          ...
          <SettingsSection
            title="Overview"
            description="Basic app information and status summary"
            compact
          >
              ...
              <div className="space-y-4">
                <div className="grid gap-3">
                  <SummaryCard
                    ...
                    description={
                      app?.published_at
                        ? `Published on ${formatDateTime(app.published_at)}`
                        : 'Unpublished'
                    }
                  ...
                  <SummaryCard
                    ...
                    value={lastRunLabel}
                    description={
                      latestExecutionAt ? 'From the latest execution' : 'No execution records'
                    }
                ...
                <div className="rounded-md bg-surface-75 border border-border p-4">
                  ...
                  <div className="mt-2 text-[11px] text-foreground-muted">
                    After updating the workflow in the Builder page, you can view run monitoring and
                    external access points here.
                  </div>
                </div>
              </div>
            </div>
          ...
          >
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <SummaryCard
                ...
                description={
                  anonymousEnabled
                    ? 'No sign-in needed to access'
                    : 'Sign-in required, or only members can access'
                }
          ...
          >
            ...
            <div className="mt-3 rounded-md border border-border bg-surface-75 px-4 py-3 text-[11px] text-foreground-muted">
              {app?.current_version?.changelog ||
                'No version change description. We recommend adding a changelog when publishing.'}
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              <Button variant="outline" size="sm" asChild>
                <Link href={`/dashboard/app/${appId}/versions`}>
          ...
          >
            ...
            {!metrics && (
              <div className="mt-3 text-[11px] text-foreground-muted">
                Metrics unavailable. Defaults shown as 0. View real-time data on the Run Monitor
                page.
              </div>
            )}
          </SettingsSection>
...
function SummaryCard({
  ...
  return (
    <div className="rounded-md bg-surface-75 border border-border p-4">
      ...
      {description && <div className="mt-2 text-[11px] text-foreground-muted">{description}</div>}
    </div>
  )
}
