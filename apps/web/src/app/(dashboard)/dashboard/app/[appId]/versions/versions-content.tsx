...
import { ArrowLeftRight, CheckCircle2, History, Loader2, RefreshCw, RotateCcw } from 'lucide-react'
...
export function VersionsPageContent({ workspaceId, appId }: VersionsPageProps) {
  ...
  const handleRollback = async (versionId: string) => {
    if (
      !confirm(
        'Are you sure you want to rollback to this version? This will overwrite the current live version.'
      )
    )
      return
    try {
  ...
  const handleRestore = async (versionId: string) => {
    if (
      !confirm(
        'Are you sure you want to restore to this version? This will overwrite the current live version.'
      )
    )
      return
    try {
  ...
  const handlePresetCompare = (versionId: string) => {
    ...
    const target =
      app?.current_version_id && app.current_version_id !== versionId
  ...
  const creatorOptions = Array.from(new Set(versions.map((version) => resolveCreator(version))))
  ...
  const activeVersion =
    versions.find((version) => version.id === activeVersionId) || currentVersion || versions[0]
    ...
    >
      <PageContainer>
        ...
        >
          ...
          {compareError && <div className="mt-3 text-[11px] text-destructive">{compareError}</div>}
          {versionDiff && (
