...
export default function AnalyticsPage() {
  ...
  const peakPoint = usageTrendData.reduce((max, item) => (item.value > max.value ? item : max))
  const averageUsage = Math.round(
    usageTrendData.reduce((sum, item) => sum + item.value, 0) / usageTrendData.length
  )
  const growthRate =
  ...
  const avgConversations = Math.round(
    dailyUsage.reduce((sum, item) => sum + item.conversations, 0) / dailyUsage.length
  )
  const avgWorkflows = Math.round(
    dailyUsage.reduce((sum, item) => sum + item.workflows, 0) / dailyUsage.length
  )
  const totalConversations = dailyUsage.reduce((sum, item) => sum + item.conversations, 0)
  const totalWorkflows = dailyUsage.reduce((sum, item) => sum + item.workflows, 0)
  ...
  return (
    <PageWithSidebar sidebar={sidebar} sidebarTitle="Analytics" sidebarWidth="narrow">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={cn(
                        ...
                        item.change >= 0 ? 'text-brand-500' : 'text-destructive'
                      )}
                    >
                      {item.change >= 0 ? '+' : ''}
                      {item.change}%
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span
                      className={cn(
                        ...
                        stat.trend === 'up' ? 'text-brand-500' : 'text-destructive'
                      )}
                    >
          ...
          <div className="rounded-md border border-border bg-surface-100 p-5">
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              <div className="rounded-md bg-surface-75 px-3 py-2">
                <p className="text-[11px] text-foreground-muted uppercase tracking-wide">Peak</p>
                <p className="text-sm font-medium text-foreground tabular-nums">
                  {peakPoint.value.toLocaleString()} · {peakPoint.label}
                </p>
              </div>
              <div className="rounded-md bg-surface-75 px-3 py-2">
                <p className="text-[11px] text-foreground-muted uppercase tracking-wide">Growth</p>
                <p
                  className={cn(
              ...
              <div className="rounded-md bg-surface-75 px-3 py-2">
                <p className="text-[11px] text-foreground-muted uppercase tracking-wide">Recent</p>
                <p className="text-sm font-medium text-foreground tabular-nums">
          ...
          <div className="rounded-md border border-border bg-surface-100 p-5">
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="flex justify-center">
                <SimplePieChart data={tokenDistribution} size={160} donut showLegend={false} />
              </div>
              <div className="space-y-3">
                {tokenDistribution.map((item) => (
                  <div key={item.label} className="flex items-center justify-between text-[12px]">
                    <span className="flex items-center gap-2 text-foreground">
                      <span className={cn('w-2 h-2 rounded-full bg-current', item.color)} />
                      {item.label}
                    </span>
                    <span className="text-foreground-light tabular-nums">{item.value}%</span>
                  </div>
                ))}
                <div className="pt-3 mt-3 border-t border-border">
                  ...
                  <div className="h-1.5 rounded-full bg-surface-300 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min((usageBudget.used / usageBudget.limit) * 100, 100)}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <Tabs defaultValue="conversations" className="w-full">
            <div className="rounded-md border border-border bg-surface-100">
              <div className="px-5 pt-4 pb-0 border-b border-border">
                <TabsList variant="underline" size="sm" className="w-full justify-start">
                ...
                {breakdownTabs.map((tab) => (
                  <TabsContent key={tab.id} value={tab.id} className="mt-0">
                    <div className="grid lg:grid-cols-[1.4fr_1fr] gap-6">
                      <div>
                        <p className="text-[12px] text-foreground-light mb-3">{tab.description}</p>
                        <SimpleBarChart
                        ...
                        <div className="mt-4 grid grid-cols-3 gap-3">
                          {tab.kpis.map((kpi) => (
                            <div key={kpi.label} className="rounded-md bg-surface-75 px-3 py-2">
                              <p className="text-[11px] text-foreground-muted uppercase tracking-wide">
                                {kpi.label}
                              </p>
                              <div className="flex items-center justify-between mt-0.5">
                                ...
                                <span
                                  className={cn(
                                    ...
                                    kpi.change >= 0 ? 'text-brand-500' : 'text-destructive'
                                  )}
                                >
                                  {kpi.change >= 0 ? '+' : ''}
                                  {kpi.change}%
                      ...
                      <div className="space-y-2">
                        ...
                        {tab.segments.map((segment) => (
                          <div key={segment.label} className="rounded-md bg-surface-75 px-3 py-2">
                ...
                {featureUsage.map((feature, index) => (
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        'text-[11px] tabular-nums',
                        feature.change >= 0 ? 'text-brand-500' : 'text-destructive'
                      )}
                    >
                      {feature.change >= 0 ? '+' : ''}
                      {feature.change}%
            ...
            <div className="rounded-md border border-border bg-surface-100 p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                ...
                <div className="rounded-md bg-surface-75 px-3 py-2">
                  ...
                  <p className="text-sm font-medium text-foreground tabular-nums">{avgWorkflows}</p>
                </div>
                <div className="rounded-md bg-surface-75 px-3 py-2">
                  <p className="text-[11px] text-foreground-muted uppercase tracking-wide">
                    Peak Day
                  </p>
                  <p className="text-sm font-medium text-foreground tabular-nums">{peakDay.day}</p>
                </div>
              </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {optimizationTips.map((tip) => (
                ...
                >
                  ...
                  <p className="text-[12px] text-foreground-light">{tip.description}</p>
                </div>
              ))}
            </div>
