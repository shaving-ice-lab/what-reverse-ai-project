...
export default function NotificationsPage() {
  ...
  const bulkDelete = async () => {
    ...
    if (failed.length > 0) {
      toast.error(
        'Failed to partially delete',
        `${failed.length} ${failed.length === 1 ? 'item' : 'items'} could not be deleted`
      )
    }
  }
  ...
  const someFilteredSelected = filteredNotifications.some((notification) =>
                ...
                <div className="py-4">
                  ...
                  <div className="mt-2 space-y-1">
                    {typeOptions.map((option) => {
                      ...
                      const badgeVariant =
                        option.value === 'all' ? 'secondary' : notificationTypes[option.value].badge
                              ...
                              <div className="flex items-center gap-1 shrink-0 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100 transition-opacity">
                                ...
                                >
                                  <Star
                                    className={cn(
                                      'w-4 h-4',
                                      notification.starred && 'fill-warning'
                                    )}
                                  />
                                </Button>
                                {!notification.read && (
