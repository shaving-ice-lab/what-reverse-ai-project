...
export default function BillingPage() {
  ...
  const [selectedInvoiceDetail, setSelectedInvoiceDetail] = useState<BillingInvoiceDetail | null>(
    null
  )
  ...
  const billingHint =
    billingCycle === 'yearly' ? 'Includes 20% discount' : 'Switch to yearly for 20% discount'
  ...
  const discountAmount = billingCycle === 'yearly' ? Math.round((currentPlan?.price ?? 0) * 0.2) : 0
  ...
  useEffect(() => {
    ...
    const loadInvoices = async () => {
      try {
        ...
        if (!isActive) return
        setInvoiceError(
          error instanceof Error ? error.message : 'Failed to fetch billing information'
        )
        setInvoices([])
    ...
    const loadWorkspaceUsageStats = async () => {
      try {
        ...
        setWorkspaceUsageError(
          error instanceof Error ? error.message : 'Failed to fetch workspace usage data'
        )
        // Example usage data
  ...
  const usageItems = [
    ...
    },
    {
      ...
      unit: 'GB',
      digits: 1,
      helper: buildUsageHelper(resolvedUsage.storage.used, resolvedUsage.storage.limit, 'GB', 1),
    },
    {
      id: 'bandwidth',
  ...
  const quotaStatusVariant =
    maxUsagePercent >= 1 ? 'error' : maxUsagePercent >= 0.8 ? 'warning' : 'success'
    ...
    <PageContainer>
      <div className="space-y-6">
        ...
        <Callout variant="info" title="Upgrade Guide">
          Your quota usage is growing rapidly with your business. We recommend evaluating an upgrade
          or requesting additional quota before limits are reached.
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline" size="sm">
              <Link href="/dashboard/support-tickets?category=billing">
            ...
            <div>
              ...
              <p className="page-panel-description">
                {currentPlan?.description ||
                  'A plan designed for professional users and small teams'}
              </p>
            </div>
              ...
              <div className="rounded-md border border-border bg-surface-75 p-4">
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-xs text-foreground-muted">
                    Can be applied to future billing
                  </span>
                </div>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
              {usageItems.map((item) => {
                ...
                return (
                  <div key={item.id} className="rounded-md border border-border bg-surface-75 p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
        ...
        <section className="page-panel">
          <div className="page-panel-header flex items-center justify-between">
            <div>
              ...
              <p className="page-panel-description">
                View usage and cost per app for analysis and optimization
              </p>
            </div>
            <div className="flex items-center gap-2">
                ...
                <tbody className="divide-y divide-border">
                  {workspaceUsageStats.length === 0 ? (
                    ...
                    workspaceUsageStats.map((stat) => (
                      <tr
                        key={stat.id}
                        className="bg-surface-75 hover:bg-surface-100 transition-colors"
                      >
                          ...
                          <div className="inline-flex items-center gap-1">
                            {stat.trend_direction === 'up' && (
                              <>
                                ...
                                <span className="text-xs text-error tabular-nums">
                                  +{stat.trend_percent}%
                                </span>
                              </>
                            )}
                  ...
                  <tfoot className="bg-surface-200/50">
                    <tr>
                      ...
                      <td className="px-4 py-2 text-right text-sm font-medium text-foreground tabular-nums">
                        {formatNumber(
                          workspaceUsageStats.reduce((sum, s) => sum + (s.usage.requests || 0), 0)
                        )}
                      </td>
                      <td className="px-4 py-2 text-right text-sm font-medium text-foreground tabular-nums">
                        {formatNumber(
                          workspaceUsageStats.reduce((sum, s) => sum + (s.usage.tokens || 0), 0)
                        )}
                      </td>
                      <td className="px-4 py-2 text-right text-sm font-medium text-foreground tabular-nums">
                        {formatNumber(
                          workspaceUsageStats.reduce((sum, s) => sum + (s.usage.storage || 0), 0),
                          1
                        )}{' '}
                        GB
                      </td>
                      <td className="px-4 py-2 text-right text-sm font-medium text-foreground tabular-nums">
                        {formatNumber(
                          workspaceUsageStats.reduce((sum, s) => sum + (s.usage.bandwidth || 0), 0),
                          1
                        )}{' '}
                        GB
                      </td>
                      <td className="px-4 py-2 text-right text-sm font-medium text-foreground tabular-nums">
                        {formatCurrency(
                          workspaceUsageStats.reduce((sum, s) => sum + s.cost_amount, 0)
                        )}
                      </td>
        ...
        <section className="page-panel">
          <div className="page-panel-header flex items-center justify-between">
            <div>
              ...
              <p className="page-panel-description">
                Manage budget controls to avoid overage costs
              </p>
            </div>
            <Button variant="outline" size="sm">
              Adjust Limits
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-foreground-muted">
                      Security threshold when quota is exceeded
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="rounded-md border border-border bg-surface-75 p-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground-muted">
                    Basic quota included in current plan
                  </p>
                </div>
              </div>
        ...
        <section className="page-panel">
          <div className="page-panel-header flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="page-panel-description">
                Select the right plan based on your team size
              </p>
            </div>
                    ...
                    {invoices.length === 0 ? (
                      ...
                      invoices.map((bill) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right">
                            <Badge variant={resolveInvoiceStatusVariant(bill.status)} size="sm">
          ...
          <section className="page-panel">
            <div className="page-panel-header flex items-center justify-between">
              <div>
                ...
                <p className="page-panel-description">
                  All billing notifications will be sent to these emails
                </p>
              </div>
              <Button variant="outline" size="sm">
                Manage Contacts
          ...
          <section className="page-panel">
            <div className="page-panel-header flex items-center justify-between">
              <div>
                ...
                <p className="page-panel-description">
                  Used for generating compliant invoices and tax information
                </p>
              </div>
              <Button variant="outline" size="sm">
                Save Changes
          ...
          <div className="p-6 flex flex-col md:flex-row md:items-center gap-4 relative">
            ...
            <div className="flex-1">
              ...
              <p className="text-description mt-1">
                For each friend you successfully invite to sign up, both of you will receive 1,000
                bonus API calls.
              </p>
            </div>
            <Button
