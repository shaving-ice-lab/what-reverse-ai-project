...
const statusVariants: Record<string, { label: string; variant: "success" | "warning" | "error" | "info" }> = {
  ...
  suspended: { label: "暂停", variant: "error" },
  deleted: { label: "删除", variant: "warning" },
  cold_storage: { label: "冷存储", variant: "info" },
  draft: { label: "草稿", variant: "info" },
  published: { label: "已发布", variant: "success" },
  deprecated: { label: "已弃用", variant: "warning" },
  archived: { label: "已归档", variant: "warning" },
  open: { label: "待处理", variant: "warning" },
  in_progress: { label: "处理中", variant: "info" },
  waiting_on_customer: { label: "等待用户", variant: "warning" },
  resolved: { label: "已解决", variant: "success" },
  closed: { label: "已关闭", variant: "error" },
};
...
> = {
  open: { label: "开始处理", nextStatus: "in_progress", variant: "default" },
  in_progress: { label: "标记解决", nextStatus: "resolved", variant: "secondary" },
  resolved: { label: "关闭工单", nextStatus: "closed", variant: "outline" },
  closed: { label: "重新打开", nextStatus: "open", variant: "default" },
};
    ...
    if (!isAdmin) {
      return (
        <div className="rounded-2xl border border-border bg-surface-100 p-8 text-center">
          ...
          <h2 className="text-lg font-semibold text-foreground">需要管理员权限</h2>
          <p className="mt-2 text-sm text-foreground-muted">
            当前账号未被授权访问管理后台，请联系系统管理员开通权限。
          </p>
        </div>
      );
    }
        ...
        <div className="relative flex flex-col gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h3 className="text-lg font-semibold text-foreground">运营与风控协同台</h3>
              <p className="mt-1 text-sm text-foreground-muted">
                统一入口管理用户、Workspace、应用处置与支持工单，所有操作可追溯。
              </p>
            </div>
          </div>
  ...
  return (
    <PageContainer className="space-y-6">
      <PageHeader
        ...
        description="管理员权限校验、风控处置、支持联动的一站式工作台。"
        actions={
          isAdmin ? (
      ...
      {isAdmin && (
        <SettingsSection
          title="管理指挥台"
          description="按模块处理运营任务，支持最小闭环的查询、处置与追踪。"
        >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" size="sm" className="flex items-center gap-1">
                          ...
                          {item.role === "admin" ? "管理员" : item.role === "creator" ? "创作者" : "用户"}
                        </Badge>
                      </div>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {users.length === 0 && !userLoading && (
                    <p className="text-sm text-foreground-muted">暂无匹配用户。</p>
                  )}
                </div>
              </div>
            </TabsContent>
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-3">
                  <Input
                    ...
                    placeholder="搜索 Workspace 名称/Slug"
                    className="max-w-xs"
                  />
                  <select
                          ...
                          <Button
                            ...
                            onClick={() =>
                              openActionDialog({
                                ...
                                id: item.id,
                                title: "暂停 Workspace",
                                description: "暂停后该 Workspace 资源将不可访问",
                                nextStatus: "suspended",
                              })
                            }
                          >
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    ...
                    <option value="waiting_on_customer">等待用户</option>
                ...
                <div className="space-y-2">
                    ...
                    return (
                      ...
                      >
                        <div className="min-w-[240px]">
                          ...
                          {latest?.at && (
                            <p className="mt-1 text-xs text-foreground-muted">
                              最近流转：{latest.from || "创建"} → {historyLabel} ·{" "}
                              {formatDate(latest.at)}
                            </p>
                          )}
                          {ticket.status_note && (
                            <p className="mt-1 text-xs text-warning">备注：{ticket.status_note}</p>
                          )}
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          {ticket.sla_response_due_at && (
                            ...
                            >
                              SLA {slaOverdue ? "已超时" : formatDate(ticket.sla_response_due_at)}
                            </Badge>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <Button asChild size="sm" variant="outline">
                            <Link href={`/support-tickets/${ticket.id}`}>查看详情</Link>
                          </Button>
                          {action && (
                            <Button
                              size="sm"
                              ...
                              onClick={() =>
                                openActionDialog({
                                  ...
                                  id: ticket.id,
                                  title: "更新工单状态",
                                  description: `将状态更新为「${statusVariants[action.nextStatus]?.label ?? action.nextStatus}」`,
                  ...
                  {tickets.length === 0 && !ticketLoading && (
                    <p className="text-sm text-foreground-muted">暂无匹配工单。</p>
                  )}
                </div>
              </div>
            </TabsContent>
      ...
      >
        <div className="space-y-2">
          <label className="text-xs text-foreground-muted">
            {actionTarget?.type === "ticket" ? "状态说明" : "处置原因"}
          </label>
          <textarea
            value={actionReason}
            onChange={(event) => setActionReason(event.target.value)}
            placeholder={actionTarget?.type === "ticket" ? "补充处理说明（可选）" : "填写处置原因（可选）"}
            className={cn(
