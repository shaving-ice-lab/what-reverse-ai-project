...
export default function TemplatesPage() {
  ...
  const getCategoryLabel = (categoryId: string) => categoryNameMap.get(categoryId) || 'Other'
    ...
    async (resetPage = false, targetPage?: number) => {
      ...
      try {
        ...
        if (resetPage) {
          ...
          setTemplates((prev) => (currentPage === 1 ? newTemplates : [...prev, ...newTemplates]))
        }
  ...
  const hasFilters = searchQuery || activeCategory !== 'all' || difficulty !== 'all'
        ...
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              <ButtonGroup attached className="border border-border rounded-md overflow-hidden">
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn(
    ...
    >
      <CardContent className="p-4 space-y-2">
        <div>
          ...
          <p className="text-xs text-foreground-light line-clamp-2 mt-1">{template.description}</p>
        </div>
  ...
  return (
    <div className="flex items-center gap-4 py-3 px-4 rounded-md bg-surface-100 border border-border hover:border-border-strong transition-colors">
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-foreground-light line-clamp-1 mt-0.5">{template.description}</p>
      </div>
