...
} from '@/components/demo/interactive-architecture'
...
export default function AppsPage() {
  ...
  const [activeScaffoldId, setActiveScaffoldId] = useState(demoScaffoldTemplates[0]?.id ?? '')
  ...
  return (
    <PageWithSidebar sidebarTitle="Workbench" sidebarWidth="wide" sidebar={sidebar}>
      <PageContainer>
                  ...
                  {workbenchRoutes.map((route) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mt-1 text-xs text-(--workbench-muted)">{route.description}</p>
              ...
              <div className="space-y-4">
                <div className="rounded-xl border border-(--workbench-border) bg-(--workbench-panel) p-4">
                  ...
                  {workspaceLoading ? (
                    ...
                    <div className="mt-3 text-xs text-(--workbench-muted)">{workspaceError}</div>
                  ) : activeWorkspace ? (
                ...
                <div className="rounded-xl border border-(--workbench-border) bg-(--workbench-panel) p-4">
                  ...
                  {workspaceAppsPreview.length > 0 ? (
                    ...
                    <div className="mt-3 text-xs text-(--workbench-muted)">
                      {activeWorkspaceId
                        ? 'No apps; create a workflow first.'
        ...
        <section
          id="sample-apps"
          className="mt-6 rounded-md border border-border bg-surface-100 p-5"
        >
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h2 className="text-sm font-medium text-foreground">{demoFlowScript.title}</h2>
            ...
            <div className="rounded-md border border-border bg-surface-75/80 p-4">
              ...
              <div className="space-y-3">
                {releaseCadencePlan.channels.map((channel) => (
                  <div
                    key={channel.id}
                    className="rounded-md border border-border/60 bg-surface-100/60 p-3"
                  >
            ...
            <div className="rounded-md border border-border bg-surface-75/80 p-4">
              ...
              <div className="space-y-2">
                {releaseCadencePlan.checklist.map((item, index) => (
                  <div
                    key={item}
                    className="flex items-center gap-2 text-[12px] text-foreground-light"
                  >
        ...
        >
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-md border border-border bg-surface-75/80 p-4">
                <div className="text-[11px] uppercase tracking-wider text-foreground-muted mb-3"></div>
                <div className="flex flex-wrap gap-2">
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-md border border-border bg-surface-75/80 overflow-hidden">
              ...
              {environmentNamingSpec.environments.map((env) => (
                ...
                >
                  ...
                  <div className="col-span-2 text-[11px] text-foreground-muted">{env.access}</div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              <div className="rounded-md border border-border bg-surface-75/80 p-4">
                ...
                <div className="mt-3 text-[11px] text-foreground-muted">
                  Config prefix:{' '}
                  {environmentNamingSpec.environments.map((env) => env.configPrefix).join(' / ')}
                </div>
              </div>
        ...
        <section id="deploy" className="mt-6 rounded-md border border-border bg-surface-100 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h2 className="text-sm font-medium text-foreground">
                {deploymentPipelineStrategy.title}
              </h2>
              <p className="text-xs text-foreground-light">
                {deploymentPipelineStrategy.description}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2 text-[11px] text-foreground-muted">
              <Badge variant="secondary" size="xs">
                ...
                <div className="space-y-3">
                  {deploymentPipelineStrategy.canary.trafficSteps.map((step) => (
                    ...
                    >
                      ...
                      <div className="text-[11px] text-foreground-muted mt-2">
                        Success Target: {step.successCriteria}
                      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-4 flex items-center justify-between text-xs text-foreground-muted">
                        <span>Workspace: {app.workspace?.name || 'Unnamed workspace'}</span>
                        <span>Updated at {formatDate(app.updatedAt)}</span>
                      </div>
