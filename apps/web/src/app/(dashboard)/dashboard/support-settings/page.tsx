...
const mergeTemplateConfig = (input?: SupportNotificationTemplateConfig) => {
  const base = JSON.parse(
    JSON.stringify(defaultTemplateConfig)
  ) as SupportNotificationTemplateConfig
  if (!input) {
    return base
  }
...
export default function SupportSettingsPage() {
  ...
  const [templateConfig, setTemplateConfig] =
    useState<SupportNotificationTemplateConfig>(mergeTemplateConfig())
  ...
  const toggleChannel = async (channel: SupportChannel) => {
    try {
      ...
      setChannels((prev) => prev.map((item) => (item.id === channel.id ? response.channel : item)))
    } catch (error) {
  ...
  const submitRule = async () => {
    ...
    try {
      if (editingRuleId) {
        ...
        setRules((prev) => prev.map((item) => (item.id === editingRuleId ? response.rule : item)))
      } else {
  ...
  const toggleRule = async (rule: SupportAssignmentRule) => {
    try {
      ...
      setRules((prev) => prev.map((item) => (item.id === rule.id ? response.rule : item)))
    } catch (error) {
  ...
  const submitTeam = async () => {
    ...
    try {
      if (editingTeamId) {
        ...
        setTeams((prev) => prev.map((item) => (item.id === editingTeamId ? response.team : item)))
      } else {
  ...
  const toggleTeam = async (team: SupportTeam) => {
    try {
      ...
      setTeams((prev) => prev.map((item) => (item.id === team.id ? response.team : item)))
    } catch (error) {
  ...
  const toggleQueue = async (queue: SupportQueue) => {
    try {
      ...
      setQueues((prev) => prev.map((item) => (item.id === queue.id ? response.queue : item)))
    } catch (error) {
    ...
    <div className="page-section p-6">
      <div className="page-header">
        <div>
          ...
          <p className="page-description">
            Manage support channels and ticket auto-dispatch policy
          </p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="page-panel-description">
                  Configure external support entry and contact method
                </p>
              </div>
              <Badge
                variant="secondary"
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-foreground-muted mt-1">
                          {channel.contact || 'No contact method specified'}
                        </p>
          ...
          <div className="page-panel-header">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="page-panel-description">
                  Auto-dispatch tickets to teams by priority and channel
                </p>
              </div>
              <Badge
                variant="secondary"
            ...
            <div className="rounded-md border border-border bg-surface-75/60 p-4 space-y-3">
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                <select
                  ...
                  onChange={(e) =>
                    setRuleForm((prev) => ({ ...prev, assigneeType: e.target.value }))
                  }
                  className="h-9 rounded-md border border-border bg-surface-200 px-2 text-xs text-foreground"
                >
                ...
                {ruleForm.assigneeType === 'team' ? (
                  <select
                    ...
                    onChange={(e) =>
                      setRuleForm((prev) => ({ ...prev, assigneeValue: e.target.value }))
                    }
                    className="h-9 rounded-md border border-border bg-surface-200 px-2 text-xs text-foreground"
                  >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            ...
                            className="bg-surface-200 text-foreground-muted text-[11px]"
                          >
                            {rule.assignee_type || 'team'} ·{' '}
                            {rule.assignee_value || 'Not configured'}
                          </Badge>
                        </div>
                        <p className="text-xs text-foreground-muted mt-1">
                          {rule.priority || 'No limit'} / {rule.category || 'No limit'} /{' '}
                          {rule.channel || 'No limit'} / {rule.keyword || 'No keywords'}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-foreground-muted">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setTeamForm((prev) => ({ ...prev, enabled: checked }))
                    }
                  />
                  Enable Team
                </div>
              ...
              <div className="space-y-3">
                {teams.map((team) => {
                  ...
                  return (
                    <div
                      key={team.id}
                      className="rounded-md border border-border bg-surface-75/60 p-4 space-y-3"
                    >
                      <div className="flex items-center justify-between">
                        <div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-foreground-muted">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setQueueForm((prev) => ({ ...prev, enabled: checked }))
                    }
                  />
                  Enable Queue
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Switch
                            ...
                            onCheckedChange={() => toggleQueue(queue)}
                          />
                          <Button
                            variant="outline"
                            size="sm"
                      ...
                      <div className="grid sm:grid-cols-3 gap-3">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => addQueueMember(queue.id)}
                        >
                          Add
                        </Button>
                      </div>
      ...
      <div className="page-panel mt-6">
        <div className="page-panel-header">
          ...
          <p className="page-panel-description">
            Configure ticket notification title and content template
          </p>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div className="text-xs text-foreground-muted">
            Available variables: {'{{reference}}'}, {'{{subject}}'}, {'{{status}}'}, {'{{note}}'},{' '}
            {'{{comment}}'}, {'{{assignee}}'}
          </div>
