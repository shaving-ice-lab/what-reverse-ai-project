...
export default function ExecutionsPage() {
  ...
  const stats = {
    ...
    successRate:
      executions.length > 0
        ? (executions.filter((e) => e.status === "completed").length / executions.filter((e) => e.status !== "pending" && e.status !== "running").length) * 100
        : 0,
    avgDuration:
  ...
  const statCards = [
    ...
    },
    {
      ...
      helper: `${stats.completed} 成功 / ${stats.failed} 失败`,
      icon: CheckCircle2,
      ...
      <div className="space-y-6">
        ...
        <section className="page-panel p-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-foreground-muted">
            <span>显示 {visibleExecutions.length} / {executions.length}</span>
            {selectedStatus !== "all" && (
              <Badge variant="outline" size="xs">
                状态：{statusConfig[selectedStatus as ExecutionStatus]?.label}
              </Badge>
            )}
            {selectedTrigger !== "all" && (
              <Badge variant="outline" size="xs">
                触发：{triggerTypes.find((t) => t.value === selectedTrigger)?.label}
              </Badge>
            )}
            {timeRange !== "all" && (
              <Badge variant="outline" size="xs">
        ...
        <section className="page-panel overflow-hidden">
          <div className="page-panel-header flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="page-panel-title">执行列表</p>
              <p className="page-panel-description">共 {visibleExecutions.length} 条执行记录</p>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="border-t border-border">
            <div className="grid items-center gap-4 px-4 py-2.5 border-b border-border bg-surface-75/80 text-table-header grid-cols-[24px_minmax(0,1fr)_80px] md:grid-cols-[24px_minmax(0,1fr)_100px_100px_80px] lg:grid-cols-[24px_minmax(0,1fr)_100px_100px_100px_80px]">
              <Checkbox
                checked={selectedItems.size === visibleExecutions.length && visibleExecutions.length > 0}
                onCheckedChange={toggleSelectAll}
              />
              <span className="text-left">工作流</span>
              <span className="hidden md:block text-center">状态</span>
              <span className="hidden md:block text-center">时长</span>
              <span className="hidden lg:block text-center">开始时间</span>
            ...
            {visibleExecutions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-base font-medium text-foreground mb-2">没有执行记录</h3>
                <p className="text-[13px] text-foreground-light mb-4 max-w-sm">
                  {searchQuery ? "尝试其他搜索条件" : "运行工作流后会显示执行记录"}
                </p>
                <Link href="/dashboard/workflows">
                  <Button size="sm" leftIcon={<Zap className="w-4 h-4" />}>
                    查看工作流
              ...
              <div className="divide-y divide-border">
                {visibleExecutions.map((exec) => {
                  ...
                  return (
                    ...
                    >
                      <Checkbox checked={isSelected} onCheckedChange={() => toggleSelect(exec.id)} />
                      ...
                      <div className="min-w-0 flex items-start gap-3">
                        ...
                        >
                          <StatusIcon
                            className={cn("w-4 h-4", config.color, exec.status === "running" && "animate-spin")}
                          />
                        </div>
                        <div className="min-w-0">
                          ...
                          {exec.error && (
                            <p className="text-[11px] text-destructive mt-1 truncate">{exec.error}</p>
                          )}
                        </div>
                      </div>

                      <div className="hidden md:flex justify-center">
                        <Badge variant={config.variant} size="sm" icon={<StatusIcon className="w-3 h-3" />}>
                          {config.label}
                        </Badge>
                      </div>
                      ...
                      <div className="hidden lg:flex justify-center">
                        <span className="text-[13px] text-foreground-light">{formatDate(exec.startedAt)}</span>
                      </div>
