...
export default function WorkspaceSettingsPage() {
  ...
  const [customTemplatesByDataset, setCustomTemplatesByDataset] = useState<
    Record<string, ReplayColumnTemplate[]>
  >({})
  const [sharedTemplatesByDataset, setSharedTemplatesByDataset] = useState<
    Record<string, ReplayColumnTemplate[]>
  >({})
  ...
  const [editingTemplateScope, setEditingTemplateScope] = useState<'personal' | 'shared' | null>(
    null
  )
  ...
  const getTemplatesForDataset = (dataset: string) => {
    ...
    const customWithFlag = customTemplates.map((item) => ({
      ...item,
      isCustom: true,
      source: 'personal' as const,
    }))
    const sharedWithFlag = sharedTemplates.map((item) => ({
      ...item,
      isCustom: true,
      ...
      .map((item) => ({
        ...
        columns: normalizeColumns(
          ...
          Array.isArray(item.columns)
            ? item.columns.filter((value: unknown) => typeof value === 'string')
            : []
        ),
        isCustom: true,
        source: item.source,
  ...
  const applyCloudReplayConfig = (config: CloudReplayConfig) => {
    ...
    datasets.forEach((dataset) => {
      ...
      if (Array.isArray(columns)) {
        const normalized = normalizeColumns(
          dataset,
          columns.filter((value) => typeof value === 'string')
        )
        if (normalized.length > 0) {
          nextColumns[dataset] = normalized
        ...
        if (sanitized.length > 0) {
          ...
          if (typeof window !== 'undefined') {
            window.localStorage.setItem(
              storageKey(dataset, 'custom_templates'),
        ...
        Object.keys(parsed?.templatesByDataset || {}).forEach((dataset) => {
          ...
          if (Array.isArray(templates)) {
            ...
            if (sanitized.length > 0) {
              ...
              if (typeof window !== 'undefined') {
                window.localStorage.setItem(
                  storageKey(dataset, 'shared_templates'),
                  JSON.stringify(sanitized)
                )
              }
            }
    ...
    if (isExecutionDataset) {
      return [
        ...
        },
        {
          ...
          accessor: 'status',
          sortable: true,
          cell: (value) => renderStatus(String(value || '')),
        },
        {
          id: 'duration_ms',
          header: 'Duration',
  ...
  const toggleVisibleColumn = (field: string) => {
    setVisibleColumnsByDataset((prev) => {
      ...
      if (typeof window !== 'undefined') {
        window.localStorage.setItem(
          storageKey(replayDataset, 'columns'),
  ...
  const handleShareTemplate = async (template: ReplayColumnTemplate) => {
    ...
    if (typeof window !== 'undefined') {
      window.localStorage.setItem(
        storageKey(replayDataset, 'shared_templates'),
      ...
      <PageContainer>
        ...
        {activeTab === 'general' && (
          <div className="space-y-6">
            ...
            >
              <FormRow label="Workspace Name" description="Name used for display">
                <Input
                  value={editForm.name}
              ...
              <FormRow label="URL Identifier" description="A unique identifier used in access URLs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 rounded-md border border-border bg-surface-75 hover:border-border-strong transition-colors">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" size="xs" onClick={() => setShowWebhookDialog(true)}>
                      Config
                    </Button>
                  </div>
                </div>
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-md bg-surface-200 flex items-center justify-center">
                              {integration.type === 'webhook' && (
                                <Webhook className="w-4 h-4 text-foreground-muted" />
                              )}
        ...
        <Dialog open={showRotateKeyDialog} onOpenChange={setShowRotateKeyDialog}>
          <DialogContent className="sm:max-w-[480px]">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleRotateApiKey} disabled={isCreatingKey || !newKeyValue.trim()}>
                {isCreatingKey ? (
                  <>
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                    Rotation...
        ...
        {activeTab === 'billing' && (
          <div className="space-y-6">
            ...
            {/* UsageStatistics */}
            <SettingsSection title="Usage Statistics" description="Current month's resource usage">
        ...
        {activeTab === 'archives' && (
          <PermissionGate
            ...
            fallback={
              ...
              >
                <div className="text-[12px] text-foreground-light">
                  You do not have permission to view log archives and replay content. Please contact
                  a workspace admin.
                </div>
              </SettingsSection>
            }
          >
            <div className="space-y-6">
              ...
              >
                <FormRow label="Archive Type" description="Select the type of log data to archive">
                  <Select
                    value={archiveType}
                    onValueChange={(value) => setArchiveType(value as LogArchiveType)}
                  >
                ...
                {archiveError && <div className="text-[12px] text-destructive">{archiveError}</div>}
              </SettingsSection>
                ...
                {isArchiveLoading ? (
                  ...
                  <div className="space-y-2">
                    {archives.map((job) => {
                      ...
                      const typeLabel =
                        archiveTypeOptions.find((item) => item.value === job.archive_type)?.label ||
                        job.archive_type
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <Badge
                                  variant="secondary"
                                  className={cn('text-[10px]', status.bg, status.color)}
                                >
                              ...
                              <div className="text-[11px] text-foreground-muted mt-1">
                                Range: {formatDateTime(job.range_start)} -{' '}
                                {formatDateTime(job.range_end)}
                              </div>
                              <div className="text-[11px] text-foreground-muted">
                                Create: {formatDateTime(job.created_at)} · Expired:{' '}
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {job.status === 'completed' && (
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleArchiveDownload(job)}
                              >
                ...
                footer={
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      ...
                      disabled={isReplayLoading || !selectedArchiveId}
                    >
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center gap-3 p-3 rounded-md bg-surface-75 border border-border">
                      ...
                      <div className="text-[12px] text-foreground">
                        {
                          archiveTypeOptions.find(
                            (item) => item.value === selectedArchive.archive_type
                          )?.label
                        }
                      </div>
                      <Badge
                        variant="secondary"
                        className="text-[10px] bg-surface-200 text-foreground-light"
                      >
                        {formatDateTime(selectedArchive.range_start)} -{' '}
                        {formatDateTime(selectedArchive.range_end)}
                      </Badge>
                    </div>

                    <FormRow label="Dataset" description="Select the archived dataset to replay">
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="text-[11px] text-foreground-muted">
                            ...
                            {cloudSyncStatus === 'error' &&
                              (cloudSyncError || 'Failed to sync cloud configuration')}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent
                              align="end"
                              className="w-56 bg-surface-100 border-border"
                            >
                              {currentFieldMappings.map((mapping) => (
                                <DropdownMenuCheckboxItem
                                  key={mapping.field}
                      ...
                      {isReplayLoading ? (
                        ...
                        <DataTable
                          ...
                          searchable
                          searchKeys={visibleReplayColumns.map(
                            (column) => column.id as keyofReplayRow
                          )}
                          paginated
                          pageSize={10}
          ...
          <div className="space-y-3 py-2">
            ...
            {sharedSyncStatus === 'error' && (
              <p className="text-[12px] text-destructive">
                {sharedSyncError || 'Failed to sync team templates'}
              </p>
            )}
      ...
      <Dialog open={showInviteDialog} onOpenChange={setShowInviteDialog}>
        <DialogContent className="sm:max-w-md bg-surface-100 border-border">
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              <label className="block text-[12px] font-medium text-foreground mb-2">Role</label>
