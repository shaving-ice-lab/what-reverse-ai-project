...
import { workspaceApi, type WorkspaceMember } from '@/lib/api/workspace'
...
const roles = [
  {
    ...
    name: 'Owner',
    description:
      'Full control permissions, including billing, security policies, and workspace deletion',
    color: 'text-warning',
  ...
  },
  {
    ...
    bgColor: 'bg-surface-200',
    icon: Eye,
    permissions: ['View apps and workflows', 'View execution records', 'View member list'],
  },
]
...
export default function WorkspaceMembersPage() {
  ...
  const [members, setMembers] =
    useState<Array<WorkspaceMember & { lastActiveAt?: string }>>(mockMembers)
  ...
  const handleRemoveMember = async (member: WorkspaceMember) => {
    if (!confirm(`Are you sure you want to remove ${member.user?.username || member.user?.email}?`))
      return
    try {
      // TODO: Connect to real API
    ...
    <PageContainer>
      <div className="space-y-6">
        ...
        <div className="p-4 rounded-md bg-surface-100 border border-border">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1 text-[12px] text-foreground-light">
              <p>
                <strong>Invite Members</strong>: Invite via email to join the workspace. Supports
                resending and expiration reminders.
              </p>
              <p>
                <strong>Role Configuration</strong>: Owner / Admin / Member / Viewer each have
                different permission scopes.
              </p>
              <p>
                <strong>Permission Details</strong>: Member management, billing, and security
                policies require Admin or Owner permissions.
              </p>
            </div>
          </div>
        </div>
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-foreground-light">{member.user?.email}</p>
                            </div>
                          </div>
                        </td>
        ...
        <SettingsSection
          ...
          description="Each role has a different permission scope"
        >
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {roles.map((role) => {
              const RoleIcon = role.icon
              return (
                <div key={role.id} className="p-4 rounded-md bg-surface-75 border border-border">
                  <div className="flex items-center gap-2 mb-3">
                    <div
                      className={cn(
