      ...
      try {
        ...
        if (active) {
          ...
          if (apps.items?.length === 0) {
            const suggestedAppName = `${active.name || "我的"} App`;
        ...
        if (mounted) setError("加载初始化信息失败，请稍后重试");
      } finally {
  ...
  return (
    <PageContainer>
      <PageHeader
        eyebrow="Setup"
        title="初始化你的工作空间"
        description="设置 Workspace 名称并创建你的第一个应用。"
      ...
      <div className="space-y-4">
        ...
        <SettingsSection
          title="Workspace 命名"
          description="为你的 Workspace 起一个易识别的名称和链接标识。"
          footer={
        ...
        >
          <div className="space-y-4">
            <FormRow label="Workspace 名称" required>
              <Input
                value={workspaceName}
                onChange={(e) => {
            ...
            <FormRow label="Workspace Slug" required description="用于链接与标识（可自动生成）">
              <Input
                value={workspaceSlug}
                onChange={(e) => {
        ...
        <SettingsSection
          title="创建第一个 App"
          description={hasApps ? "检测到已有 App，可直接继续。" : "创建一个应用，用于开始你的第一个项目。"}
        >
          <div className={cn("space-y-4", hasApps && "opacity-60")}>
            <FormRow label="App 名称" required={!hasApps}>
              <Input
                value={appName}
                onChange={(e) => {
                ...
                placeholder="例如：客服自动化"
                disabled={hasApps}
              />
            </FormRow>
            <FormRow label="App Slug" required={!hasApps} description="用于链接与标识（可自动生成）">
              <Input
                value={appSlug}
                onChange={(e) => {
            ...
            <FormRow label="App 描述" description="可选，用于说明应用用途。">
              <Input
                value={appDescription}
                onChange={(e) => setAppDescription(e.target.value)}
                placeholder="例如：自动回复常见问题"
                disabled={hasApps}
              />
            </FormRow>
          </div>
