...
export function useWebSocket(options: UseWebSocketOptions = {}) {
  ...
  const calculateLatency = useCallback((message: WSMessage) => {
    ...
    metricsRef.current.avgLatencyMs = Math.round(
      totalLatencyRef.current / metricsRef.current.totalMessages
    )
    ...
    if (latencyMs > LATENCY_THRESHOLD_MS) {
      ...
      console.warn(
        `[WebSocket] Message latency exceeded threshold: ${latencyMs}ms (threshold: ${LATENCY_THRESHOLD_MS}ms)`,
        {
          type: message.type,
          latencyMs,
        }
