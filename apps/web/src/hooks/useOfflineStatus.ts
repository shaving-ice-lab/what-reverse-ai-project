...
export function useOfflineStatus(options: UseOfflineStatusOptions = {}): UseOfflineStatusReturn {
  ...
  // Network
  const [quality, setQuality] = useState<NetworkQuality | undefined>(() => getNetworkQuality())
  ...
  // QueueStatus
  const [queueState, setQueueState] = useState<OfflineQueueState>(() => queueRef.current.getState())
  ...
  const handleConnectionChange = useCallback(() => {
    ...
    if (autoSync && onProcess && newStatus === 'online' && connectionInfo.status === 'slow') {
      queueRef.current.process(onProcess).then(updateQueueState)
    }
