...
export class SyncEngine {
  ...
  constructor(localStorage: LocalStorage, cloudApi: CloudApi, config: Partial<SyncConfig> = {}) {
  ...
  async getState(): Promise<SyncEngineState> {
    ...
    return {
      ...
      nextSyncAt: this.syncInterval ? new Date(Date.now() + this.config.interval) : undefined,
    }
  }
  ...
  async sync(): Promise<SyncResult> {
    ...
    try {
      ...
      const result: SyncResult = {
        status:
          uploadResults.failed.length > 0 || downloadResults.failed.length > 0
            ? 'partial'
            : 'success',
    ...
    for (const conflict of conflicts) {
      ...
      switch (this.config.conflictResolution) {
        ...
        case 'cloud':
          ...
          break
        case 'latest':
          const comparison = compareTimestamps(conflict.local.timestamp, conflict.cloud.timestamp)
