...
import type { NodeContext, NodeResult, NodeExecutor, VariableConfig } from '../types'
import { renderTemplate, getValueByPath, safeJSONParse, createNodeError } from '../utils'
...
export const variableExecutor: NodeExecutor<VariableConfig> = {
  ...
  async execute(context): Promise<NodeResult> {
    ...
    try {
      ...
      // Parse variable value
      const parsedValue = parseVariableValue(nodeConfig.value, nodeConfig.valueType, allVariables)
    ...
    try {
      ...
      switch (nodeConfig.transformType) {
        ...
        case 'expression': {
          ...
          const expr = renderTemplate(nodeConfig.expression, {
            ...variables,
            ...inputs,
            data: inputData,
          })
...
export const databaseExecutor: NodeExecutor<DatabaseConfig> = {
  ...
  async execute(context): Promise<NodeResult> {
    ...
    try {
      ...
      if (operation === 'select') {
        const rowsSource = nodeConfig.mockRows ?? inputs.rows ?? inputs.output ?? inputs.input
