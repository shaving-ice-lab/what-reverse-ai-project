...
function evaluateConditionGroup(
  ...
  const results = group.conditions.map((condition) => evaluateCondition(condition, variables))
 ...
 */
function evaluateConditions(config: ConditionConfig, variables: Record<string, unknown>): boolean {
  const results = config.conditions.map((group) => evaluateConditionGroup(group, variables))
