...
import type { NodeContext, NodeResult, NodeExecutor, TemplateConfig } from '../types'
  ...
  async execute(context): Promise<NodeResult> {
    ...
    try {
      ...
      switch (nodeConfig.extractType) {
        ...
        case 'matchAll': {
          const matches = [
            ...inputText.matchAll(
              new RegExp(
                pattern,
                nodeConfig.flags?.includes('g') ? nodeConfig.flags : (nodeConfig.flags || '') + 'g'
              )
            ),
          ]
          result = matches.map((m) =>
            nodeConfig.groupIndex !== undefined ? m[nodeConfig.groupIndex] : m[0]
          )
          break
        }
        ...
        case 'replace': {
          const replacement = renderTemplate(nodeConfig.replacement || '', {
            ...variables,
            ...inputs,
          })
