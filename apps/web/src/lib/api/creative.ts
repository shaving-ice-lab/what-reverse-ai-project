...
export async function exportDocument(id: string, format: ExportFormat): Promise<Blob> {
  const baseUrl = getApiBaseUrl()
  const response = await fetch(`${baseUrl}/creative/documents/${id}/export?format=${format}`, {
    method: 'GET',
    credentials: 'include',
  })
...
export async function regenerateSection(
  ...
  return api.post<RegenerateSectionResponse>(`/creative/documents/${documentId}/regenerate`, data)
}
...
export async function createShare(data: CreateShareRequest): Promise<CreateShareResponse> {
  return api.post<CreateShareResponse>(`/creative/documents/${data.documentId}/share`, {
...
export async function getSharedDocument(
  ...
  const query = searchParams.toString()
  return api.get<CreativeDocument>(`/creative/share/${shareId}${query ? `?${query}` : ''}`)
}
