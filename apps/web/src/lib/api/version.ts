...
export const versionApi = {
  ...
  async list(
    ...
    return request<VersionListResponse>(
      `/workflows/${workflowId}/versions${query ? `?${query}` : ''}`
    )
  },

  /**
   * FetchSpecifyVersionDetails
   */
  ...
  /**
   * RestoretoSpecifyVersion
   */
  async restore(workflowId: string, version: number, changeLog?: string): Promise<RestoreResponse> {
   ...
   * forcompareVersion
   */
  async compare(workflowId: string, v1: number, v2: number): Promise<CompareResponse> {
    return request<CompareResponse>(`/workflows/${workflowId}/versions/compare?v1=${v1}&v2=${v2}`)
  },

  /**
   * ManualCreateVersionSnapshot
   */
  async createSnapshot(workflowId: string, changeLog: string): Promise<VersionResponse> {
