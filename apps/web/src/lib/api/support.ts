...
export const supportApi = {
  ...
  adminUpdateRule(
    ...
    return api.patch<{ rule: SupportAssignmentRule }>(
      `/admin/support/routing-rules/${ruleId}`,
      payload
    )
  },
  ...
  adminAddTeamMember(
    teamId: string,
    payload: { user_id: string; role?: string; sort_order?: number }
  ) {
    return api.post<{ member: SupportTeamMember }>(
      `/admin/support/teams/${teamId}/members`,
      payload
    )
  },
  ...
  adminAddQueueMember(queueId: string, payload: { user_id: string; sort_order?: number }) {
    return api.post<{ member: SupportQueueMember }>(
      `/admin/support/queues/${queueId}/members`,
      payload
    )
  },
