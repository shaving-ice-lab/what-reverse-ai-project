...
function extractErrorInfo(payload: Partial<ApiResponse<unknown>> | undefined) {
  ...
  const message =
    ...
    'Request failed'
  const code =
    (fallback.error_code as string) || payload?.code || (error?.code as string) || 'REQUEST_FAILED'
...
async function refreshAccessToken(): Promise<boolean> {
  ...
  try {
    ...
    const data: ApiResponse<{ access_token: string; refresh_token: string }> = await response
      .json()
      .catch(() => ({}) as ApiResponse<{ access_token: string; refresh_token: string }>)
...
async function executeFetch(
  ...
  if (fetchConfig.signal) {
    if (fetchConfig.signal.aborted) {
      ...
      fetchConfig.signal.addEventListener('abort', () => controller.abort(), { once: true })
    }
  }
...
async function performRequest<T>(
  ...
  if (response.status === 401 && headers.has('Authorization')) {
    ...
    const retryData: ApiResponse<T> = await retryResponse.json().catch(() => ({}) as ApiResponse<T>)
...
export async function requestRaw<T>(endpoint: string, config: RequestConfig = {}): Promise<T> {
  const {
    params,
  ...
  const maxRetries =
    typeof retry === 'number' ? retry : ['GET', 'HEAD', 'OPTIONS'].includes(method) ? 1 : 0
