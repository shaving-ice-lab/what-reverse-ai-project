...
export const planApi = {
  ...
  async createVersion(workspaceId: string, data: { label?: string; note?: string } = {}): Promise<PlanVersion> {
  ...
  async getVersion(versionId: string): Promise<PlanVersion> {
    const response = await request<ApiResponse<{ version: PlanVersion }>>(`/plans/versions/${versionId}`);
    return response.data.version;
  },
  ...
  async getIncidentDrillPlans(): Promise<IncidentDrillPlanSet> {
    const response = await request<ApiResponse<{ plan: IncidentDrillPlanSet }>>(
      "/plans/incident-drills"
    );
    return response.data.plan;
  },

  async getIncidentOwnerTable(): Promise<IncidentOwnerTable> {
    const response = await request<ApiResponse<{ table: IncidentOwnerTable }>>(
      "/plans/incident-owners"
    );
  ...
  async getOnCallSLOTable(): Promise<OnCallSLOTable> {
    const response = await request<ApiResponse<{ table: OnCallSLOTable }>>(
      "/plans/sre/oncall-slo"
    );
    return response.data.table;
  },
