 ...
 * Decompress snapshot
 */
export async function decompressSnapshot(snapshot: ExecutionSnapshot): Promise<ExecutionSnapshot> {
...
export function createSnapshotFromExecution(
  ...
  const metadata: SnapshotMetadata = {
    ...
    version: '1.0.0',
    compressed: false,
    source: typeof window !== 'undefined' && '__TAURI__' in window ? 'desktop' : 'web',
  }
 ...
 */
function createNodeSnapshotFromState(nodeState: NodeExecutionState): NodeSnapshot {
  return {
 ...
 */
function calculateSummary(nodeSnapshots: Record<string, NodeSnapshot>): ExecutionSummary {
  ...
  return {
    ...
    totalTokensUsed: nodes.reduce((sum, n) => sum + (n.metadata?.tokensUsed || 0), 0),
  }
}
  ...
  switch (nodeType) {
    ...
    case 'http':
      return metadata?.httpStatusCode
        ...
        : 'HTTP Request'
    case 'condition':
      return metadata?.conditionBranch ? `Branch: ${metadata.conditionBranch}` : 'Condition check'
    case 'loop':
