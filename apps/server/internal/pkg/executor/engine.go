...
func NewEngine(cfg *EngineConfig, log logger.Logger, dbProvider DBProvider, dbAuthorizer DBAuthorizer) *Engine {
	...
	engine.RegisterExecutor(NewFormSubmitExecutor())
	engine.RegisterExecutor(NewPageRenderExecutor())
	engine.RegisterExecutor(NewAggregateExecutor())
	engine.RegisterExecutor(NewNotificationExecutor())
	engine.RegisterExecutor(NewScheduleTriggerExecutor())
