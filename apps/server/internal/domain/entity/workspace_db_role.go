...
type WorkspaceDBRole struct {
	...
	WorkspaceID   uuid.UUID  `gorm:"type:char(36);not null;index;uniqueIndex:uniq_workspace_app_role_status" json:"workspace_id"`
	AppID         *uuid.UUID `gorm:"type:char(36);index;uniqueIndex:uniq_workspace_app_role_status" json:"app_id,omitempty"`
	RoleType      string     `gorm:"size:20;not null;index;uniqueIndex:uniq_workspace_app_role_status" json:"role_type"`
	DBUser        string     `gorm:"column:db_user;size:100;not null" json:"db_user"`
	SecretRef     *string    `gorm:"column:secret_ref;size:200" json:"-"`
	Status        string     `gorm:"size:20;default:'active';index;uniqueIndex:uniq_workspace_app_role_status" json:"status"`
